    ...
    while (it.next()) |entry| {
        io.print(
            ...
            .{
                ...
                self.gc.get(o.ObjTypeDef, entry.value_ptr.*).toStringAlloc(self.gc.allocator, false, self.gc) catch unreachable,
            },
        );
    }
    io.print("===========================\n\n", .{});
...
pub fn getTypeDef(self: *TypeRegistry, type_def: o.ObjTypeDef) !*o.ObjTypeDef {
    ...
    if (type_def.def_type != .Placeholder) {
        if (self.registry.get(hash)) |type_def_ptr| {
            return self.gc.get(o.ObjTypeDef, type_def_ptr);
        }
    }
    ...
    if (type_def.resolved_type) |resolved| {
        switch (resolved) {
            ...
            .EnumInstance => {
                ...
                hashHelper(gc, hasher, gc.get(o.ObjTypeDef, resolved.EnumInstance.of));
            },
            .Fiber => {
                hashHelper(gc, hasher, gc.get(o.ObjTypeDef, resolved.Fiber.return_type));
                hashHelper(gc, hasher, gc.get(o.ObjTypeDef, resolved.Fiber.yield_type));
            },
            ...
            .Function => {
                ...
                hashHelper(gc, hasher, gc.get(o.ObjTypeDef, resolved.Function.return_type));
                hashHelper(gc, hasher, gc.get(o.ObjTypeDef, resolved.Function.yield_type));
                if (resolved.Function.error_types) |types| {
                    for (types) |error_type| {
                        hashHelper(gc, hasher, gc.get(o.ObjTypeDef, error_type));
                    }
                }

                {
                    ...
                    for (parameter_types) |parameter_type| {
                        hashHelper(gc, hasher, gc.get(o.ObjTypeDef, parameter_type));
                    }
                }
                ...
                if (resolved.Function.resolved_generics) |types| {
                    for (types) |gen_type| {
                        hashHelper(gc, hasher, gc.get(o.ObjTypeDef, gen_type));
                    }
                }
            },
            ...
            .Object => {
                if (resolved.Object.anonymous) {
                    ...
                    while (it.next()) |kv| {
                        ...
                        hashHelper(gc, hasher, gc.get(o.ObjTypeDef, kv.value_ptr.type_def));
                    }
                } else {
                    ...
                    if (resolved.Object.resolved_generics) |rg| {
                        for (rg) |gen| {
                            hashHelper(gc, hasher, gc.get(o.ObjTypeDef, gen));
                        }
                    }
                }
            },
            .ObjectInstance => {
                ...
                hashHelper(gc, hasher, gc.get(o.ObjTypeDef, resolved.ObjectInstance.of));
            },
            ...
            .ProtocolInstance => {
                ...
                hashHelper(gc, hasher, gc.get(o.ObjTypeDef, resolved.ProtocolInstance.of));
            },
        }
    }
}
