...
chunk_count: usize = 0,
elements: std.ArrayList(Ast.String.Element),
line_offset: usize,
column_offset: usize,
...
pub fn init(
    ...
    return Self{
        ...
        .elements = std.ArrayList(Ast.String.Element).init(parser.gc.allocator),
...
fn push(self: *Self, chars: []const u8) !void {
    try self.elements.append(
        .{
            .element = try self.parser.ast.appendNode(
                .{
                    .tag = .StringLiteral,
...
fn interpolation(self: *Self) !void {
    // Parse interpolation format
    const fmt = if (self.source[self.offset] == '%') format: {
        _ = self.advance();

        break :format try Ast.String.parsePlaceholder(self.source[self.offset..]);
    } else null;
    ...
    try self.elements.append(
        .{
            .element = try self.parser.expression(false),
            .format = fmt,
        },
    );
