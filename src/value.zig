...
pub const Value = extern struct {
    ...
    pub fn typeOf(self: Value, gc: *GC) !*o.ObjTypeDef {
        ...
        if (self.isDouble()) {
            return gc.get(o.ObjTypeDef, gc.type_registry.double_type);
        }

        if (self.isInteger()) {
            return gc.get(o.ObjTypeDef, gc.type_registry.int_type);
        }

        return switch (self.getTag()) {
            TagBoolean => gc.get(o.ObjTypeDef, gc.type_registry.bool_type),
            TagNull, TagVoid => gc.get(o.ObjTypeDef, gc.type_registry.void_type),
            TagSentinel => unreachable,
            else => gc.get(o.ObjTypeDef, gc.type_registry.double_type),
        };
    }
