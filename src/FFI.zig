...
fn identifier(self: *Self, decl_index: Ast.Node.Index) Error!*Zdef {
    ...
    if ((type_def == null or zig_type == null) and self.state.?.parser != null) {
        ...
        if (global != null and global.?.type_def.get(self.gc).def_type == .ForeignContainer) {
            type_def = global.?.type_def.get(self.gc).*;
            zig_type = type_def.?.resolved_type.?.ForeignContainer.zig_type;
        }

        if (global == null) {
            if (self.state.?.structs.get(id)) |container| {
                type_def = container.type_def.get(self.gc).*;
...
fn ptrType(self: *Self, tag: Ast.Node.Tag, decl_index: Ast.Node.Index) Error!*Zdef {
    ...
    if (try self.getZdef(ptr_type.ast.child_type)) |child_type| {
        ...
        else if (child_type.type_def.get(self.gc).def_type == .ForeignContainer)
            .{
