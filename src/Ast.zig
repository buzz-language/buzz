...
pub const Slice = struct {
    ...
    /// ctx should have:
    /// - `fn processNode(ctx: @TypeOf(ctx), gc: *GC, allocator: std.mem.Allocator, ast: Ast.Slice, node: Node.Index) error{OutOfMemory}!bool: returns true to stop walking deeper
    // FIXME: the context should have a error type we can reuse in processNode signature
    pub fn walk(self: Slice, gc: *GC, arena: std.mem.Allocator, ctx: anytype, root: Node.Index) !void {
        ...
        while (node_queue.items.len > 0) {
            ...
            if (try ctx.processNode(gc, arena, self, node) and node_queue.items.len == 0) {
                return;
            }
    ...
    const UsesFiberContext = struct {
        ...
        pub fn processNode(self: *UsesFiberContext, _: *GC, _: std.mem.Allocator, ast: Self.Slice, node: Self.Node.Index) (std.mem.Allocator.Error || std.fmt.BufPrintError)!bool {
    ...
    pub fn usesFiber(self: Self.Slice, gc: *GC, allocator: std.mem.Allocator, node: Node.Index) !bool {
        var ctx = UsesFiberContext{};

        try self.walk(gc, allocator, &ctx, node);
    ...
    const IsConstantContext = struct {
        ...
        pub fn processNode(self: *IsConstantContext, gc: *GC, _: std.mem.Allocator, ast: Self.Slice, node: Self.Node.Index) (std.mem.Allocator.Error || std.fmt.BufPrintError)!bool {
            switch (ast.nodes.items(.tag)[node]) {
                .AnonymousObjectType,
                .FiberType,
                .FunctionType,
                ...
                .Dot => {
                    const type_def = ast.nodes.items(.type_def)[ast.nodes.items(.components)[node].Dot.callee].?.get(gc);
                ...
                .List => {
                    ...
                    if (ast.nodes.items(.type_def)[node].?.get(gc)
    ...
    pub fn isConstant(self: Self.Slice, gc: *GC, allocator: std.mem.Allocator, node: Node.Index) !bool {
    ...
    pub fn toValue(
        ...
        if (value.* == null and try self.isConstant(gc, gc.allocator, node)) {
            value.* = switch (self.nodes.items(.tag)[node]) {
                ...
                .Dot => dot: {
                    ...
                    break :dot .fromObj(
                        .{
                            .index = (try gc.allocateObject(
                                obj.ObjEnumInstance{
                                    .enum_ref = type_def.get(gc).resolved_type.?.Enum.value.?,
                ...
                .List => list: {
                    ...
                    std.debug.assert(
                        type_def_idx.get(gc).def_type != .Placeholder,
                    );
                    ...
                    for (list_components.items) |item| {
                        try list_idx.get(gc).items.append(
                            gc.allocator,
                            try self.toValue(item, reporter, gc),
                        );
                    }

                    break :list .fromObj(.{ .index = list_idx.index, .obj_type = .List });
                },
                .Map => map: {
                    const map_components = components[node].Map;
                    const type_def_idx = self.nodes.items(.type_def)[node].?;

                    std.debug.assert(type_def_idx.get(gc).def_type != .Placeholder);
                    ...
                    break :map .fromObj(.{ .index = map_idx.index, .obj_type = .Map });
                },
                .String => string: {
                ...
                .Subscript => subscript: {
                    ...
                    switch (subscriptable.obj_type) {
                        .List => {
                            const list = gc.ptr(obj.ObjList, .idx(subscriptable.index)).?;
                        ...
                        .Map => {
                            const map = gc.ptr(obj.ObjMap, .idx(subscriptable.index)).?;

                            break :subscript map.map.get(key) orelse Value.Null;
                        },
                        .String => {
                            const str = gc.ptr(obj.ObjString, .idx(subscriptable.index)).?;
                            const index: usize = @intCast(key.integer());
                ...
                .Unary => unary: {
                    ...
                    break :unary switch (unary_components.operator) {
                        .Bnot => .fromInteger(~val.integer()),
                        .Bang => .fromBoolean(!val.boolean()),
