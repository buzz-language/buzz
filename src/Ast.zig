...
pub const Slice = struct {
        ...
        pub fn processNode(
            ...
            switch (ast.nodes.items(.tag)[node]) {
                ...
                .List => {
                    ...
                    if (self.gc.get(obj.ObjTypeDef, ast.nodes.items(.type_def)[node].?).resolved_type.?.List.mutable) {
                        self.result = false;
                        return true;
                    }
                ...
                .Map => {
                    ...
                    if (self.gc.get(obj.ObjTypeDef, ast.nodes.items(.type_def)[node].?).resolved_type.?.Map.mutable) {
                        self.result = false;
                        return true;
                    }
    ...
    fn matchConditionValue(
        ...
        const condition_type_def = gc.get(obj.ObjTypeDef, self.nodes.items(.type_def)[condition].?);
        ...
        if (value.* == null and try self.isConstant(gc.allocator, gc, node)) {
            value.* = switch (self.nodes.items(.tag)[node]) {
                ...
                .AnonymousEnumCase => enum_case: {
                    ...
                    const type_def = gc.get(obj.ObjTypeDef, self.nodes.items(.type_def)[node].?);
                    ...
                    const enum_type_def = gc.get(
                        obj.ObjTypeDef,
                        type_def.resolved_type.?.EnumInstance.of,
                    ).resolved_type.?.Enum;
                ...
                .Dot => dot: {
                    ...
                    const type_def = gc.get(obj.ObjTypeDef, self.nodes.items(.type_def)[dot_components.callee].?);
                ...
                .Match => match: {
                    ...
                    const value_type_def = gc.get(
                        obj.ObjTypeDef,
                        self.nodes.items(.type_def)[match_components.value].?,
                    );
                ...
                .List => list: {
                    ...
                    std.debug.assert(type_def != null and gc.get(obj.ObjTypeDef, type_def.?).def_type != .Placeholder);

                    var list = try gc.allocateObject(
                        try obj.ObjList.init(gc.allocator, gc.get(obj.ObjTypeDef, type_def.?)),
                    );
                ...
                .Map => map: {
                    ...
                    var map = try gc.allocateObject(
                        try obj.ObjMap.init(gc.allocator, gc.get(obj.ObjTypeDef, type_def.?)),
                    );
    ...
    pub const BlockExpressionFlow = struct {
        ...
        fn mergeOutType(self: *BlockExpressionFlow, gc: *GC, type_def: ?obj.ObjTypeDef.Idx) Error!void {
            ...
            if (self.out_type) |current| {
                if (!gc.get(obj.ObjTypeDef, current).eql(gc.get(obj.ObjTypeDef, incoming))) {
    ...
    fn constantBoolean(
        ...
        const type_def_ptr = gc.get(obj.ObjTypeDef, type_def);
