...
pub fn onDispatch(self: *Debugger) Error!bool {
    ...
    if (filled) {
        if (self.out_poller.reader(0).buffered().len > 0) {
            self.adapter.emitEvent(
                .{
                    .event = .output,
                    .body = .{
                        .output = .{
                            .category = .stdout,
                            .output = self.out_poller.reader(0).buffered(),
                        },
                    },
                },
            );
        ...
        if (self.out_poller.reader(1).buffered().len > 0) {
            self.adapter.emitEvent(
                .{
                    .event = .output,
                    .body = .{
                        .output = .{
                            .category = .stderr,
                            .output = self.out_poller.reader(1).buffered(),
                        },
                    },
                },
            );
