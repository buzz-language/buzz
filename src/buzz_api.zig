        ...
        switch (value.obj(vm.gc).obj_type) {
            ...
            .List => {
                ...
                print(
                    ...
                    "{s}[ ",
                    .{
                        if (vm.gc.get(o.ObjTypeDef, list.type_def).resolved_type.?.List.mutable)
                            "mut "
                        else
                            "",
                    },
            ...
            .Map => {
                ...
                print(
                    ...
                    "{s}{{ ",
                    .{
                        if (vm.gc.get(o.ObjTypeDef, map.type_def).resolved_type.?.Map.mutable)
                            "mut "
                        else
                            "",
                    },
            ...
            .Enum => {
                ...
                const enum_type_def = vm.gc.get(o.ObjTypeDef, enumeration.type_def).resolved_type.?.Enum;

                print(
                    io,
                    "enum({s}) {s} {{ ",
                    .{
                        vm.gc.get(o.ObjString, enum_type_def.name).string,
                        vm.gc.get(o.ObjString, enum_type_def.name).string,
                    },
                );
            ...
            .Object => {
                ...
                const object_def = vm.gc.get(o.ObjTypeDef, object.type_def).resolved_type.?.Object;
                ...
                if (object_def.conforms_to.count() > 0) {
                    ...
                    while (it.next()) |kv| {
                        print(io, "{s}, ", .{vm.gc.get(o.ObjString, vm.gc.get(o.ObjTypeDef, kv.key_ptr.*).resolved_type.?.Protocol.name).string});
                    }
                    print(io, ")", .{});
                }

                print(io, " {s} {{ ", .{vm.gc.get(o.ObjString, object_def.name).string});

                {
                    var it = object_def.fields.iterator();
                    while (it.next()) |kv| {
                        const field = kv.value_ptr.*;
                        const field_type_str = vm.gc.get(o.ObjTypeDef, field.type_def)
            ...
            .ObjectInstance => {
                ...
                const object_def = vm.gc.get(
                    o.ObjTypeDef,
                    vm.gc.get(o.ObjTypeDef, object_instance.type_def).resolved_type.?.ObjectInstance.of,
                ...
                print(
                    ...
                    "{s}{s}{{ ",
                    .{
                        if (vm.gc.get(o.ObjTypeDef, object_instance.type_def).resolved_type.?.ObjectInstance.mutable)
                            "mut "
                        else
                            "",
                        if (object_instance.object) |object|
                            vm.gc.get(o.ObjString, vm.gc.get(o.ObjTypeDef, vm.gc.get(o.ObjObject, object).type_def).resolved_type.?.Object.name).string
                        else
                            ".",
                    },
                );
            ...
            .ForeignContainer => {
                ...
                const foreign_def = vm.gc.get(o.ObjTypeDef, foreign.type_def).resolved_type.?.ForeignContainer;

                print(
                    io,
                    "{s}{{ ",
                    .{vm.gc.get(o.ObjString, foreign_def.name).string},
                );
...
export fn bz_stringType(vm: *VM) callconv(.c) v.Value {
    return vm.gc.get(o.ObjTypeDef, vm.gc.type_registry.str_type).toValue();
}

export fn bz_intType(vm: *VM) callconv(.c) v.Value {
    return vm.gc.get(o.ObjTypeDef, vm.gc.type_registry.int_type).toValue();
}
    ...
    return switch (obj.obj_type) {
        .Bound => bound: {
            ...
            if (bound.closure) |cls| {
                break :bound vm.gc.get(o.ObjFunction, vm.gc.get(o.ObjClosure, cls).function).native != null;
            }

            break :bound false;
        },
        .Closure => vm.gc.get(o.ObjFunction, o.ObjClosure.cast(obj).?.function).native != null,
        .Native => true,
        else => false,
    };
}
...
export fn bz_newQualifiedObjectInstance(self: *VM, qualified_name: [*]const u8, len: usize, mutable: bool) callconv(.c) v.Value {
    ...
    const instance: *o.ObjObjectInstance = self.gc.allocateObject(
        o.ObjObjectInstance.init(
            self,
            object,
            self.gc.get(o.ObjTypeDef, object.type_def).toInstance(
...
fn instanciateError(
    ...
    if (message) |msg| {
        ...
        const object_def = vm.gc.get(
            o.ObjTypeDef,
            vm.gc.get(o.ObjTypeDef, obj_instance.type_def).resolved_type.?.ObjectInstance.of,
        ...
        if (global.isObj()) {
            switch (global.obj(self.gc).obj_type) {
                .Enum => {
                    ...
                    if (std.mem.eql(u8, qualified_name[0..len], self.gc.get(o.ObjString, self.gc.get(o.ObjTypeDef, obj_enum.type_def).resolved_type.?.Enum.qualified_name).string)) {
                        return global;
                    }
                },
                .Object => {
                    ...
                    if (std.mem.eql(u8, qualified_name[0..len], self.gc.get(o.ObjString, self.gc.get(o.ObjTypeDef, obj_enum.type_def).resolved_type.?.Object.qualified_name).string)) {
                        return global;
                    }
                },
                else => {},
...
export fn bz_getObjectInstanceMethod(instance_value: v.Value, method_idx: usize, bind: bool, vm: *VM) callconv(.c) v.Value {
    ...
    const method = vm.gc.get(o.ObjObject, instance.object.?).fields[method_idx];
...
export fn bz_getProtocolMethod(instance_value: v.Value, method_name: v.Value, vm: *VM) callconv(.c) v.Value {
    ...
    const method_idx = vm.gc.get(
        o.ObjTypeDef,
        vm.gc.get(o.ObjTypeDef, instance.type_def).resolved_type.?.ObjectInstance.of,
    ).resolved_type.?.Object.fields.get(name).?.index;

    return bz_bindMethod(
        vm,
        instance_value,
        vm.gc.get(o.ObjObject, instance.object.?).fields[method_idx],
        v.Value.Null,
    );
}
...
export fn bz_getEnumCase(enum_value: v.Value, case_name_value: v.Value, vm: *VM) callconv(.c) v.Value {
    ...
    for (vm.gc.get(o.ObjTypeDef, self.type_def).resolved_type.?.Enum.cases, 0..) |enum_case, index| {
...
export fn bz_getEnumInstanceValue(enum_instance_value: v.Value, vm: *VM) callconv(.c) v.Value {
    ...
    return vm.gc.get(o.ObjEnum, instance.enum_ref).cases[instance.case];
}
...
export fn bz_context(ctx: *o.NativeCtx, closure_value: v.Value, new_ctx: *o.NativeCtx, arg_count: usize) callconv(.c) *anyopaque {
    ...
    const closure = if (bound) |bd|
        if (bd.closure) |cls| ctx.vm.gc.get(o.ObjClosure, cls) else null
    else
        o.ObjClosure.cast(closure_value.obj(ctx.vm.gc));

    const native = if (closure == null and bound != null)
        if (bound.?.native) |nat| ctx.vm.gc.get(o.ObjNative, nat) else null
    ...
    if (BuildOptions.recursive_call_limit) |recursive_call_limit| {
        ...
        ctx.vm.current_fiber.recursive_count = if (closure != null and ctx.vm.gc.get(o.ObjFunction, closure.?.function) == ctx.vm.current_fiber.current_compiled_function)
    ...
    if (closure) |cls| {
        if (ctx.vm.gc.get(o.ObjFunction, cls.function).native_raw == null) {
        ...
        ctx.vm.current_fiber.current_compiled_function = ctx.vm.gc.get(o.ObjFunction, cls.function);

        return ctx.vm.gc.get(o.ObjFunction, cls.function).native_raw orelse @as(*anyopaque, @ptrFromInt(@intFromPtr(&bz_callFromJit)));
    }
...
export fn bz_closure(
    ...
    const obj_function = ctx.vm.gc.get(o.ObjFunction, ctx.vm.current_ast.nodes.items(.components)[function_node].Function.function.?);
...
export fn bz_zigType(vm: *VM, ztype: [*]const u8, len: usize, expected_type: *v.Value) callconv(.c) ?*const ZigType {
    ...
    if (zdef) |uzdef| {
        expected_type.* = vm.gc.get(o.ObjTypeDef, uzdef.type_def).toValue();
...
export fn bz_foreignContainerGet(value: v.Value, field_idx: usize, vm: *VM) callconv(.c) v.Value {
    ...
    return vm.gc.get(o.ObjTypeDef, container.type_def).resolved_type.?
        .ForeignContainer
...
export fn bz_foreignContainerSet(value: v.Value, field_idx: usize, new_value: v.Value, vm: *VM) callconv(.c) void {
    ...
    return vm.gc.get(o.ObjTypeDef, container.type_def).resolved_type.?.ForeignContainer
        .fields.values()[field_idx]
        .setter(
        vm,
        container.data.ptr,
