            ...
            switch (value.obj(state.vm.gc).obj_type) {
                ...
                .List => {
                    ...
                    out.print(
                        "{s}[{s}",
                        .{
                            if (state.vm.gc.get(obj.ObjTypeDef, list.type_def).resolved_type.?.List.mutable)
                                "mut "
                            else
                                "",
                            if (has_items)
                ...
                .Map => {
                    ...
                    out.print(
                        "{s}{{{s}",
                        .{
                            if (state.vm.gc.get(obj.ObjTypeDef, map.type_def).resolved_type.?.Map.mutable)
                                "mut "
                            else
                                "",
                            if (has_items)
                ...
                .Enum => {
                    ...
                    const enum_type = state.vm.gc.get(obj.ObjTypeDef, enumeration.type_def);
                    const enum_type_def = enum_type.resolved_type.?.Enum;
                    const enum_value_type_def = state.vm.gc.get(
                        obj.ObjTypeDef,
                        enum_type.resolved_type.?.Enum.enum_type,
                    ).toStringAlloc(state.vm.gc.allocator, true, state.vm.gc) catch unreachable;
                    ...
                    out.print(
                        ...
                        .{
                            enum_value_type_def,
                            state.vm.gc.get(obj.ObjString, enum_type_def.name).string,
                        },
                    ) catch unreachable;
                    state.tab += 1;
                ...
                .Object => {
                    ...
                    const object_def = state.vm.gc.get(obj.ObjTypeDef, object.type_def).resolved_type.?.Object;
                    ...
                    out.print(" {s} {{\n", .{state.vm.gc.get(obj.ObjString, object_def.name).string}) catch unreachable;
                    state.tab += 1;

                    var it = object_def.fields.iterator();
                    while (it.next()) |kv| {
                        const field = kv.value_ptr.*;
                        const field_type_str = state.vm.gc.get(
                            obj.ObjTypeDef,
                            field.type_def,
                        ).toStringAlloc(state.vm.gc.allocator, true, state.vm.gc) catch unreachable;
                ...
                .ObjectInstance => {
                    ...
                    const object_instance_type = state.vm.gc.get(obj.ObjTypeDef, object_instance.type_def);
                    const fields = state.vm.gc.get(
                        obj.ObjTypeDef,
                        object_instance_type.resolved_type.?.ObjectInstance.of,
                    ...
                    out.print(
                        ...
                        .{
                            ...
                            if (object_instance.object) |object|
                                state.vm.gc.get(obj.ObjString, state.vm.gc.get(
                                    obj.ObjTypeDef,
                                    state.vm.gc.get(
                                        obj.ObjObject,
                                        object,
                                    ).type_def,
                                ).resolved_type.?.Object.name).string
                            else
                                ".",
                        },
                    ) catch unreachable;
                ...
                .ForeignContainer => {
                    ...
                    const foreign_def = state.vm.gc.get(
                        obj.ObjTypeDef,
                        foreign.type_def,
                    ...
                    out.print(
                        "{s}{{\n",
                        .{state.vm.gc.get(obj.ObjString, foreign_def.name).string},
                    ) catch unreachable;
