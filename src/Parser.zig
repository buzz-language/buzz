    ...
    pub fn resolveGeneric(self: Frame, gc: *GC, name: *obj.ObjString) ?*obj.ObjTypeDef {
        if (self.generics) |generics| {
            if (generics.get(name.toIdx())) |type_def| {
                return gc.get(obj.ObjTypeDef, type_def);
            }
        }
...
fn beginFrame(self: *Self, function_type: obj.ObjFunction.FunctionType, function_node: Ast.Node.Index, this: ?*obj.ObjTypeDef) !void {
    ...
    switch (function_type) {
        ...
        .EntryPoint, .ScriptEntryPoint => {
            ...
            local.type_def = self.gc.get(obj.ObjTypeDef, self.gc.type_registry.void_type);
        },
    }
...
fn endFrame(self: *Self) Ast.Node.Index {
    ...
    if (self.reporter.last_error == null) {
        ...
        const function_type = self.gc.get(
            obj.ObjTypeDef,
            self.ast.nodes.items(.type_def)[current.function_node].?,
...
fn parsePrecedence(self: *Self, precedence: Precedence, hanging: bool) Error!Ast.Node.Index {
        ...
        if (self.opt_jumps) |*jumps| {
            ...
            if (@intFromEnum(getRule(self.ast.tokens.items(.tag)[self.current_token.?]).precedence) < @intFromEnum(jumps.items[0])) {
                ...
                if (node_type_def_ptr.* != null) {
                    node_type_def_ptr.* = (try self.gc.get(
                        obj.ObjTypeDef,
                        node_type_def_ptr.*.?,
                    ).cloneOptional(&self.gc.type_registry)).toIdx();
                }
            }
        }
    ...
    if (self.opt_jumps) |*jumps| {
        ...
        if (node_type_def != null) {
            self.ast.nodes.items(.type_def)[node] = (try self.gc.get(
                obj.ObjTypeDef,
                node_type_def.?,
            ).cloneOptional(&self.gc.type_registry)).toIdx();
        }
...
fn resolveGeneric(self: *Self, name: *obj.ObjString) ?*obj.ObjTypeDef {
    return if (self.current_object != null and self.current_object.?.generics != null)
        if (self.current_object.?.generics.?.get(name.toIdx())) |type_def|
            self.gc.get(obj.ObjTypeDef, type_def)
        else
    ...
    switch (relation) {
        ...
        .GenericResolve => {
            ...
            for (child_placeholder.resolved_generics.?, 0..) |generic, idx| {
                resolved_generics[idx] = self.gc.get(obj.ObjTypeDef, generic);
            }
        ...
        .Call => {
            ...
            try self.resolvePlaceholder(
                child,
                self.gc.get(obj.ObjTypeDef, resolved_type.resolved_type.?.Function.return_type),
                false,
            );
        },
        .Yield => {
            ...
            try self.resolvePlaceholder(
                child,
                self.gc.get(obj.ObjTypeDef, resolved_type.resolved_type.?.Function.yield_type),
                false,
            );
        },
        .Subscript => {
        ...
        .Key => {
            if (resolved_type.def_type == .Map) {
                ...
                try self.resolvePlaceholder(
                    child,
                    self.gc.get(obj.ObjTypeDef, self.gc.type_registry.int_type),
                    false,
                );
            } else {
                self.reporter.reportErrorFmt(
            ...
            switch (resolved_type.def_type) {
                ...
                .Object => {
                    ...
                    if (object_def.fields.get(child_placeholder_name)) |field| {
                        ...
                        try self.resolvePlaceholder(
                            child,
                            self.gc.get(obj.ObjTypeDef, field.type_def),
                            field.final,
                        );
                    } else {
                        self.reporter.reportErrorFmt(
                ...
                .ObjectInstance => {
                    ...
                    const object_def = self.gc.get(
                        obj.ObjTypeDef,
                        resolved_type.resolved_type.?.ObjectInstance.of,
                ...
                .ForeignContainer => {
                    ...
                    if (f_def.buzz_type.get(child_placeholder_name)) |field| {
                        try self.resolvePlaceholder(child, self.gc.get(obj.ObjTypeDef, field), false);
                    } else {
                        self.reporter.reportErrorFmt(
                            .property_does_not_exists,
                            self.ast.tokens.get(child.resolved_type.?.Placeholder.where),
                ...
                .ProtocolInstance => {
                    // We can't create a field access placeholder without a name
                    const protocol_def = self.gc.get(
                        obj.ObjTypeDef,
                        resolved_type.resolved_type.?.ProtocolInstance.of,
                    ).resolved_type.?.Protocol;

                    // Search for a field matching the placeholder
                    if (protocol_def.methods.get(child_placeholder_name)) |method_def| {
                        try self.resolvePlaceholder(child, self.gc.get(obj.ObjTypeDef, method_def.type_def), true);
                    } else {
                        self.reporter.reportErrorFmt(
                            .property_does_not_exists,
                            self.ast.tokens.get(child.resolved_type.?.Placeholder.where),
                    ...
                    if (std.mem.eql(u8, "value", child_placeholder_name)) {
                        try self.resolvePlaceholder(
                            ...
                            self.gc.get(
                                ...
                                self.gc.get(
                                    obj.ObjTypeDef,
                                    resolved_type.resolved_type.?.EnumInstance.of,
...
pub fn resolvePlaceholder(self: *Self, placeholder: *obj.ObjTypeDef, resolved_type: *obj.ObjTypeDef, final: bool) Error!void {
    ...
    for (placeholder_def.children.items) |child| {
        if (self.gc.get(obj.ObjTypeDef, child).def_type == .Placeholder) {
            try self.resolvePlaceholderWithRelation(
                self.gc.get(obj.ObjTypeDef, child),
                placeholder,
                final,
                self.gc.get(obj.ObjTypeDef, child).resolved_type.?.Placeholder.parent_relation.?,
            );
        }
    }
...
pub fn parseTypeDefFrom(self: *Self, source: []const u8) Error!*obj.ObjTypeDef {
    ...
    return self.gc.get(obj.ObjTypeDef, self.ast.nodes.items(.type_def)[parsed_type].?);
}
...
fn parseTypeDef(
    ...
    if (try self.match(.Str)) {
        ...
        if (instance) {
            self.ast.nodes.items(.type_def)[type_def_node] = (try self.gc.get(
                obj.ObjTypeDef,
        ...
        if (self.resolveGeneric(try self.gc.copyString(identifier_lexeme))) |generic_type| {
            ...
            if (generic_types.?.get((try self.gc.copyString(identifier_lexeme)).toIdx())) |generic_type| {
                user_type = self.gc.get(obj.ObjTypeDef, generic_type);
            }
        }
        ...
        if (user_type == null) {
            ...
            user_type = self.gc.get(obj.ObjTypeDef, self.ast.nodes.items(.type_def)[user_type_node.?].?);
        }
        ...
        .{
            ...
            .resolved_type = .{
                .List = obj.ObjList.ListDef.init(
                    self.gc.get(obj.ObjTypeDef, self.ast.nodes.items(.type_def)[item_type].?),
                    mutable,
                ),
            },
        },
                ...
                .{
                    ...
                    .resolved_type = .{
                        .Map = obj.ObjMap.MapDef.init(
                            self.gc.get(obj.ObjTypeDef, type_defs[key_type].?),
                            self.gc.get(obj.ObjTypeDef, type_defs[value_type].?),
                            mutable,
                        ),
                    },
                },
...
fn parseFunctionType(self: *Self, parent_generic_types: ?GenericTypeMap) Error!Ast.Node.Index {
    ...
    while (!self.check(.RightParen) and !self.check(.Eof)) {
        ...
        if (try self.match(.Equal)) {
            ...
            if (expr_type_def != null and
                self.gc.get(
                    obj.ObjTypeDef,
                    expr_type_def.?,
                ).def_type == .Placeholder and
                self.gc.get(
                    obj.ObjTypeDef,
                    arg_type_def.?,
                ).def_type == .Placeholder)
            {
                try obj.PlaceholderDef.link(
                    self.gc.allocator,
                    self.gc.get(obj.ObjTypeDef, arg_type_def.?),
                    self.gc.get(obj.ObjTypeDef, expr_type_def.?),
                    .Assignment,
                );
            }
        ...
        } else if (self.gc.get(obj.ObjTypeDef, arg_type_def.?).optional) {
            try defaults.put(
                self.gc.allocator,
                (try self.gc.copyString(arg_name)).toIdx(),
                Value.Null,
    ...
    if (try self.match(.BangGreater)) {
        ...
        while (!self.check(.Eof)) {
            ...
            const error_type_def = self.gc.get(obj.ObjTypeDef, self.ast.nodes.items(.type_def)[error_type].?);
...
fn parseGenericResolve(self: *Self, callee_type_def: *obj.ObjTypeDef, expr: ?Ast.Node.Index) Error!Ast.Node.Index {
    ...
    while (!self.check(.Greater) and !self.check(.Eof)) {
        ...
        try resolved_generics_types.append(
            self.gc.allocator,
            self.gc.get(
                obj.ObjTypeDef,
                self.ast.nodes.items(.type_def)[resolved_generic].?,
            ),
        );
...
fn subscript(self: *Self, can_assign: bool, subscripted: Ast.Node.Index) Error!Ast.Node.Index {
    ...
    const subscript_type_def = self.gc.get(
        obj.ObjTypeDef,
        self.ast.nodes.items(.type_def)[subscripted] orelse self.gc.type_registry.any_type,
    );
    ...
    const type_defs = self.ast.nodes.items(.type_def);
    if (subscript_type_def.def_type == .Placeholder and self.gc.get(
        obj.ObjTypeDef,
        index_type_def.?,
    ).def_type == .Placeholder) {
        try obj.PlaceholderDef.link(
            self.gc.allocator,
            self.gc.get(obj.ObjTypeDef, type_defs[subscripted].?),
            self.gc.get(obj.ObjTypeDef, type_defs[index].?),
            .Key,
        );
    }
    ...
    if (type_defs[subscripted]) |type_def| {
        const resolved_type_def = self.gc.get(obj.ObjTypeDef, type_def);
        ...
        try self.matchOpEqual())
    {
        ...
        if (subscript_type_def.def_type == .Placeholder and self.gc.get(
            ...
            try obj.PlaceholderDef.link(
                self.gc.allocator,
                subscript_type_def,
                self.gc.get(obj.ObjTypeDef, value_type_def.?),
                .Subscript,
            );
        }
    }
...
fn list(self: *Self, _: bool) Error!Ast.Node.Index {
    ...
    if (try self.match(.Less)) {
        ...
        item_type = self.gc.get(obj.ObjTypeDef, self.ast.nodes.items(.type_def)[explicit_item_type.?].?);
            ...
            if (item_type == null) {
                if (common_type == null) {
                    common_type = if (self.ast.nodes.items(.type_def)[actual_item]) |actual_type_def|
                        self.gc.get(obj.ObjTypeDef, actual_type_def)
                    else
                        null;
                } else if (self.ast.nodes.items(.type_def)[actual_item]) |actual_type_def| {
                    const resolved_actual_type_def = self.gc.get(obj.ObjTypeDef, actual_type_def);
                    if (!common_type.?.eql(resolved_actual_type_def)) {
                        if (common_type.?.def_type == .ObjectInstance and resolved_actual_type_def.def_type == .ObjectInstance) {
                            common_type = if (self.gc.get(
                                obj.ObjTypeDef,
                                common_type.?.resolved_type.?.ObjectInstance.of,
                            ).resolved_type.?.Object.bothConforms(
                                self.gc.get(
                                    obj.ObjTypeDef,
                                    resolved_actual_type_def.resolved_type.?.ObjectInstance.of,
                                ).resolved_type.?.Object,
                                self.gc,
                            )) |protocol_type_def|
                            ...
                            common_type = self.gc.get(obj.ObjTypeDef, self.gc.type_registry.any_type);
                        }
                    }
                }
            }
        ...
        .{
            ...
            .type_def = (try self.gc.type_registry.getTypeDef(
                .{
                    ...
                    .resolved_type = .{
                        .List = obj.ObjList.ListDef.init(
                            item_type orelse self.gc.get(obj.ObjTypeDef, self.gc.type_registry.any_type),
...
fn callExpression(self: *Self, callee: Ast.Node.Index, anon_obj_single_argument: ?Ast.Node.Index) Error!Ast.Node.Index {
    ...
    var type_def = if (callee_type_def != null and self.gc.get(obj.ObjTypeDef, callee_type_def.?).def_type == .Function)
        self.gc.get(obj.ObjTypeDef, self.gc.get(obj.ObjTypeDef, callee_type_def.?).resolved_type.?.Function.return_type)
    else if (callee_type_def != null and self.gc.get(obj.ObjTypeDef, callee_type_def.?).def_type == .Enum)
        try (try self.gc.get(obj.ObjTypeDef, callee_type_def.?).toInstance(&self.gc.type_registry, false))
    ...
    if (type_def == null) {
        if (callee_type_def == null or self.gc.get(obj.ObjTypeDef, callee_type_def.?).def_type != .Placeholder) {
            self.reportErrorAtNode(
                .callable,
                callee,
                "Can't be called",
            ...
            type_def = self.gc.get(obj.ObjTypeDef, self.gc.type_registry.void_type);
        } else {
            type_def = try self.gc.type_registry.getTypeDef(
                .{
                    .def_type = .Placeholder,
            ...
            try obj.PlaceholderDef.link(
                self.gc.allocator,
                self.gc.get(obj.ObjTypeDef, callee_type_def.?),
                type_def.?,
                .Call,
            );
        }
...
fn map(self: *Self, _: bool) Error!Ast.Node.Index {
    ...
    if (try self.match(.Less)) {
        ...
        key_type_def = self.gc.get(obj.ObjTypeDef, self.ast.nodes.items(.type_def)[key_type_node.?].?);

        try self.consume(.Colon, "Expected `:` after key type");

        value_type_node = try self.parseTypeDef(null, true);
        value_type_def = self.gc.get(obj.ObjTypeDef, self.ast.nodes.items(.type_def)[value_type_node.?].?);
        ...
        while (!(try self.match(.RightBrace)) and !(try self.match(.Eof))) {
            ...
            if (key_type_node == null) {
                if (common_key_type == null) {
                    common_key_type = self.gc.get(obj.ObjTypeDef, self.ast.nodes.items(.type_def)[key].?);
                    ...
                    if (!common_key_type.?.eql(resolved_actual_type_def)) {
                        ...
                        {
                            common_key_type = if (self.gc.get(
                                obj.ObjTypeDef,
                                common_key_type.?.resolved_type.?.ObjectInstance.of,
                            ).resolved_type.?.Object
                                .bothConforms(
                                self.gc.get(
                                    obj.ObjTypeDef,
                            ...
                            common_key_type = self.gc.get(obj.ObjTypeDef, self.gc.type_registry.any_type);
                        }
                    }
                }
            }
            ...
            if (value_type_node == null) {
                if (common_value_type == null) {
                    common_value_type = self.gc.get(obj.ObjTypeDef, self.ast.nodes.items(.type_def)[value].?);
                    ...
                    if (!common_value_type.?.eql(resolved_actual_type_def)) {
                        if (common_value_type.?.def_type == .ObjectInstance and
                            resolved_actual_type_def.def_type == .ObjectInstance)
                        {
                            common_value_type = if (self.gc.get(
                                obj.ObjTypeDef,
                                common_value_type.?.resolved_type.?.ObjectInstance.of,
                            ).resolved_type.?.Object.bothConforms(
                                self.gc.get(
                                    obj.ObjTypeDef,
                                    resolved_actual_type_def.resolved_type.?.ObjectInstance.of,
                            ...
                            common_value_type = self.gc.get(obj.ObjTypeDef, self.gc.type_registry.any_type);
                        }
                    } else {
        ...
        .{
            ...
            .type_def = (try self.gc.type_registry.getTypeDef(
                .{
                    ...
                    .resolved_type = .{
                        .Map = obj.ObjMap.MapDef.init(
                            key_type_def orelse self.gc.get(obj.ObjTypeDef, self.gc.type_registry.any_type),
                            value_type_def orelse self.gc.get(obj.ObjTypeDef, self.gc.type_registry.any_type),
...
fn objectInit(self: *Self, _: bool, object: Ast.Node.Index) Error!Ast.Node.Index {
    ...
    while (!self.check(.RightBrace) and !self.check(.Eof)) {
        ...
        if (object_type_def != null and self.gc.get(obj.ObjTypeDef, object_type_def.?).def_type == .Placeholder) {
            ...
            try obj.PlaceholderDef.link(
                self.gc.allocator,
                self.gc.get(obj.ObjTypeDef, obj_type_def.?),
    ...
    return self.ast.appendNode(
        .{
            ...
            .type_def = if (self.ast.nodes.items(.type_def)[object]) |type_def|
                (try self.gc.get(obj.ObjTypeDef, type_def).toInstance(
                    &self.gc.type_registry,
...
fn dot(self: *Self, can_assign: bool, callee: Ast.Node.Index) Error!Ast.Node.Index {
    ...
    if (callee_type_def != null) {
        const callee_def_type = self.gc.get(obj.ObjTypeDef, callee_type_def.?).def_type;
        switch (callee_def_type) {
            .String => {
                if (try obj.ObjString.memberDefByName(self, member_name)) |member_type_def| {
                    ...
                    const member = if (generic_resolve) |gr|
                        self.gc.get(obj.ObjTypeDef, self.ast.nodes.items(.type_def)[gr].?)
                    else
                        member_type_def;
            ...
            .Object => {
                const obj_def = self.gc.get(obj.ObjTypeDef, callee_type_def.?).resolved_type.?.Object;
                ...
                if (property_type == null and self.current_object != null and std.mem.eql(
                    ...
                    const kv = try self.gc.get(
                        obj.ObjTypeDef,
                        callee_type_def.?,
                    ).resolved_type.?.Object.static_placeholders
                ...
                const generic_resolve = if (try self.match(.DoubleColon))
                    try self.parseGenericResolve(
                        if (property_type) |ptype|
                            self.gc.get(obj.ObjTypeDef, ptype)
                        else
                            self.gc.get(obj.ObjTypeDef, self.gc.type_registry.any_type),
                        null,
                    )
                else
                    null;
                ...
                if (can_assign and try self.matchOpEqual()) {
                    ...
                    dot_node,
                    if (property_type) |ptype| self.gc.get(obj.ObjTypeDef, ptype) else null,
                    can_assign,
                ))) { // access only
            ...
            .ForeignContainer => {
                const f_def = self.gc.get(obj.ObjTypeDef, callee_type_def.?).resolved_type.?.ForeignContainer;
            ...
            .ObjectInstance => {
                ...
                if (property_type == null and self.current_object != null and std.mem.eql(
                    ...
                    self.reporter.reportErrorFmt(
                        ...
                        .{ member_name, self.gc.get(obj.ObjString, obj_def.name).string },
                    );
                }
            ...
            .ProtocolInstance => {
                const protocol = self.gc.get(
                    obj.ObjTypeDef,
                    self.gc.get(
                        obj.ObjTypeDef,
                        callee_type_def.?,
                    ).resolved_type.?.ProtocolInstance.of,
                );
                ...
                const generic_resolve = if (try self.match(.DoubleColon))
                    try self.parseGenericResolve(self.gc.get(obj.ObjTypeDef, method_type.?), null)
                else
                    null;
                ...
                if (!(try self.finishDotCallIfPresent(
                    dot_node,
                    if (method_type) |mtype| self.gc.get(obj.ObjTypeDef, mtype) else null,
                    can_assign,
                ))) {
                    components[dot_node].Dot.member_kind = .Ref;
            ...
            .Enum => {
                const enum_def = self.gc.get(obj.ObjTypeDef, callee_type_def.?).resolved_type.?.Enum;
            ...
            .EnumInstance => {
                ...
                self.ast.nodes.items(.components)[dot_node].Dot.member_kind = .Ref;
                self.ast.nodes.items(.type_def)[dot_node] = self.gc.get(
                    obj.ObjTypeDef,
                    self.gc.get(obj.ObjTypeDef, callee_type_def.?).resolved_type.?.EnumInstance.of,
                ).resolved_type.?.Enum.enum_type;
            },
            .List => {
                if (try obj.ObjList.ListDef.member(
...
fn unwrap(self: *Self, force: bool, unwrapped: Ast.Node.Index) Error!Ast.Node.Index {
    const unwrapped_type_def = self.gc.get(
        obj.ObjTypeDef,
        self.ast.nodes.items(.type_def)[unwrapped] orelse self.gc.type_registry.any_type,
    );
...
fn genericResolve(self: *Self, _: bool, expr: Ast.Node.Index) Error!Ast.Node.Index {
    return try self.parseGenericResolve(
        self.gc.get(obj.ObjTypeDef, self.ast.nodes.items(.type_def)[expr] orelse self.gc.type_registry.any_type),
        expr,
    );
}
...
fn @"if"(self: *Self, is_statement: bool, loop_scope: ?LoopScope) Error!Ast.Node.Index {
    ...
    if (try self.match(.Arrow)) { // if (opt -> unwrapped)
        ...
        _ = try self.parseVariable(
            @intCast(node_slot),
            self.current_token.? - 1,
            try self.gc.get(obj.ObjTypeDef, condition_type_def.?).cloneNonOptional(&self.gc.type_registry),
            true,
            self.gc.get(obj.ObjTypeDef, condition_type_def.?).isMutable(),
        ...
        _ = try self.parseVariable(
            ...
            try self.gc.get(obj.ObjTypeDef, self.ast.nodes.items(.type_def)[casted_type.?].?).toInstance(
                &self.gc.type_registry,
                self.gc.get(obj.ObjTypeDef, condition_type_def.?).isMutable(),
            ),
            true,
    ...
    self.ast.nodes.set(
        ...
        .{
            ...
            .type_def = if (!is_statement) type_def: {
                ...
                const if_type_def = if (body_type_def == null or self.gc.get(
                    obj.ObjTypeDef,
                    body_type_def.?,
                ).def_type == .Void)
                ...
                const is_optional = self.gc.get(obj.ObjTypeDef, if_type_def.?).optional or
                    self.gc.get(obj.ObjTypeDef, body_type_def.?).optional or
                    self.gc.get(obj.ObjTypeDef, else_branch_type_def.?).optional or
                    self.gc.get(obj.ObjTypeDef, body_type_def.?).def_type == .Void or
                    self.gc.get(obj.ObjTypeDef, else_branch_type_def.?).def_type == .Void;
                if (is_optional and !self.gc.get(obj.ObjTypeDef, if_type_def.?).optional) {
                    break :type_def (try self.gc.get(
                        obj.ObjTypeDef,
                        if_type_def.?,
                    ).cloneOptional(&self.gc.type_registry)).toIdx();
                }
            ...
            .type_def = type_def: {
                ...
                for (branches.items) |branch| {
                    ...
                    if (branch_type_def) |btd| {
                        const resolved_btd = self.gc.get(obj.ObjTypeDef, btd);
                        ...
                        if (resolved_btd.def_type == .Void) {
                            ...
                            if (previous.def_type == .Void) {
                                ...
                                type_def = self.gc.get(obj.ObjTypeDef, self.gc.type_registry.any_type);
                                break;
                            }
                        } else {
                            type_def = resolved_btd;
...
fn isAs(self: *Self, left: Ast.Node.Index, is_expr: bool, force: bool) Error!Ast.Node.Index {
    ...
    const type_def = self.gc.get(obj.ObjTypeDef, self.ast.nodes.items(.type_def)[constant].?);
...
fn function(
    ...
    if (function_type == .Test) {
        ...
        while (!self.check(.RightParen) and !self.check(.Eof)) {
            ...
            const argument_type = self.gc.get(obj.ObjTypeDef, self.ast.nodes.items(.type_def)[argument_type_node].?);
            ...
            const default = switch (function_type) {
                .Function, .Method, .Anonymous, .Extern => value: {
                    if (try self.match(.Equal)) {
                        ...
                        if (expr_type_def != null and self.gc.get(
                            obj.ObjTypeDef,
                            expr_type_def.?,
                        ).def_type == .Placeholder and argument_type.def_type == .Placeholder) {
                            try obj.PlaceholderDef.link(
                                self.gc.allocator,
                                argument_type,
                                self.gc.get(obj.ObjTypeDef, expr_type_def.?),
    ...
    if (function_type.canHaveErrorSet() and (try self.match(.BangGreater))) {
        ...
        while (!self.check(end_token) and !self.check(.DoubleArrow) and !self.check(.Eof)) {
            ...
            const error_type = self.gc.get(obj.ObjTypeDef, self.ast.nodes.items(.type_def)[error_type_node].?);
    ...
    if (function_type == .Extern) {
        if (self.flavor.resolveDynLib() and self.flavor.resolveImports()) {
            const native_opt = try self.importStaticLibSymbol(
                self.script_name,
                self.gc.get(obj.ObjString, function_typedef.resolved_type.?.Function.name).string,
            ) orelse
                if (!is_wasm)
...
fn asyncCall(self: *Self, _: bool) Error!Ast.Node.Index {
    ...
    const function_type = self.gc.get(obj.ObjTypeDef, call_components.Call.callee_type_def);
...
fn resumeFiber(self: *Self, _: bool) Error!Ast.Node.Index {
    ...
    if (fiber_type == null) {
        unreachable;
    } else if (self.gc.get(obj.ObjTypeDef, fiber_type.?).def_type == .Placeholder) {
        ...
        try obj.PlaceholderDef.link(
            self.gc.allocator,
            self.gc.get(obj.ObjTypeDef, fiber_type.?),
            yield_placeholder,
            .Yield,
        );
        ...
        if (self.gc.get(obj.ObjTypeDef, fiber_type.?).def_type != .Fiber) {
            self.reportErrorAtNode(
                .resumable,
                fiber_node,
                "Can't be resumed",
            ...
            const fiber = self.gc.get(obj.ObjTypeDef, fiber_type.?).resolved_type.?.Fiber;
...
fn resolveFiber(self: *Self, _: bool) Error!Ast.Node.Index {
    ...
    if (fiber_type == null) {
        ...
        if (self.gc.get(obj.ObjTypeDef, fiber_type.?).def_type != .Fiber) {
            ...
            const fiber_def = self.gc.get(obj.ObjTypeDef, fiber_type.?).resolved_type.?.Fiber;
...
fn mutableExpression(self: *Self, _: bool) Error!Ast.Node.Index {
    ...
    self.ast.nodes.items(.type_def)[expr] = (try self.gc.get(
        obj.ObjTypeDef,
        self.ast.nodes.items(.type_def)[expr].?,
    ).cloneMutable(&self.gc.type_registry, true)).toIdx();
...
fn binary(self: *Self, _: bool, left: Ast.Node.Index) Error!Ast.Node.Index {
    ...
    const null_coalescing_type_def = if (right_type_def != null and self.gc.get(
        obj.ObjTypeDef,
        right_type_def.?,
    ).def_type != .Placeholder)
        right_type_def
    else if (left_type_def != null and self.gc.get(obj.ObjTypeDef, left_type_def.?).def_type != .Placeholder)
        (try self.gc.get(obj.ObjTypeDef, left_type_def.?).cloneNonOptional(&self.gc.type_registry)).toIdx()
    else
        right_type_def;
...
fn funDeclaration(self: *Self) Error!Ast.Node.Index {
    ...
    const fun_typedef = self.gc.get(obj.ObjTypeDef, self.ast.nodes.items(.type_def)[function_node].?);
    ...
    if (is_main) {
        ...
        if (fun_def.parameters.count() > 1 or
            (self.gc.get(obj.ObjTypeDef, fun_def.return_type).def_type != .Integer and
                self.gc.get(obj.ObjTypeDef, fun_def.return_type).def_type != .Void))
        {
            ...
            if (first_param == null or
                !(try self.parseTypeDefFrom("[str]")).eql(self.gc.get(obj.ObjTypeDef, first_param.?)))
            {
                signature_valid = false;
            }
        }
...
fn objectDeclaration(self: *Self) Error!Ast.Node.Index {
    ...
    if (try self.match(.Less)) {
        while (!self.check(.Greater) and !self.check(.Eof)) : (protocol_count += 1) {
            ...
            const protocol = self.gc.get(obj.ObjTypeDef, self.ast.nodes.items(.type_def)[protocol_node].?);
    ...
    while (!self.check(.RightBrace) and !self.check(.Eof)) {
        ...
        if (try self.match(.Fun)) {
            ...
            const method_type_def = self.gc.get(obj.ObjTypeDef, self.ast.nodes.items(.type_def)[method_node].?);
            ...
            if (static) {
                if (object_type.resolved_type.?.Object.static_placeholders.get(method_name)) |placeholder| {
                    try self.resolvePlaceholder(
                        self.gc.get(obj.ObjTypeDef, placeholder.placeholder),
                        method_type_def,
                        true,
                    );
            ...
            try properties_type.put(
                self.gc.allocator,
                method_name,
                self.gc.get(obj.ObjTypeDef, self.ast.nodes.items(.type_def)[method_node].?),
            );
        } else { // Property
            const final = try self.match(.Final);
            ...
            if (static) {
                if (object_type.resolved_type.?.Object.static_placeholders.get(property_lexeme)) |placeholder| {
                    try self.resolvePlaceholder(
                        self.gc.get(obj.ObjTypeDef, placeholder.placeholder),
                        self.gc.get(obj.ObjTypeDef, property_type_def.?),
                        false,
                    );
            ...
            const default = if (try self.match(.Equal))
                try self.expression(false)
            else if (self.gc.get(obj.ObjTypeDef, property_type_def.?).optional)
                try self.ast.appendNode(
                    .{
                        .tag = .Null,
                        .location = self.current_token.? - 1,
            ...
            try properties_type.put(
                self.gc.allocator,
                property_lexeme,
                self.gc.get(obj.ObjTypeDef, self.ast.nodes.items(.type_def)[property_type].?),
            );
        }
    }
...
fn method(self: *Self, abstract: bool, static: bool, this: *obj.ObjTypeDef) Error!Ast.Node.Index {
    ...
    if (self.gc.get(obj.ObjTypeDef, self.ast.nodes.items(.type_def)[func].?).resolved_type.?.Function.lambda) {
...
fn enumDeclaration(self: *Self) Error!Ast.Node.Index {
    ...
    const enum_case_type = if (enum_case_type_node) |enum_type|
        try self.gc.get(obj.ObjTypeDef, self.ast.nodes.items(.type_def)[enum_type].?).toInstance(
            &self.gc.type_registry,
            false,
        )
    else
...
fn varDeclaration(
    ...
    var var_type = if (parsed_type) |ptype|
        try self.gc.get(obj.ObjTypeDef, self.ast.nodes.items(.type_def)[ptype].?).toInstance(
            &self.gc.type_registry,
            self.gc.get(obj.ObjTypeDef, self.ast.nodes.items(.type_def)[ptype].?).isMutable(), // ???
        )
    else
        self.gc.get(
            obj.ObjTypeDef,
            self.gc.type_registry.any_type,
        ); // When var type omitted, will be replaced by the value type bellow
    ...
    if (should_assign and value == null and
        (parsed_type == null or
            !self.gc.get(
                obj.ObjTypeDef,
                self.ast.nodes.items(.type_def)[parsed_type.?].?,
            ).optional))
    {
        const location = self.ast.tokens.get(self.current_token.? - 1);
        self.reporter.reportErrorAt(
    ...
    if (var_type.def_type == .Placeholder and
        value != null and
        value_type_def != null and
        self.gc.get(
            obj.ObjTypeDef,
            value_type_def.?,
        ).def_type == .Placeholder)
    ...
    if (parsed_type == null and value != null and value_type_def != null) {
        var_type = self.gc.get(obj.ObjTypeDef, value_type_def.?);
    ...
    if (value) |uvalue| {
        ...
        if (parsed_type_def != null and
            self.gc.get(obj.ObjTypeDef, parsed_type_def.?).def_type == .List and
        ...
        if (parsed_type_def != null and
            self.gc.get(obj.ObjTypeDef, parsed_type_def.?).def_type == .Map and
            tags[uvalue] == .Map and
...
fn testStatement(self: *Self) Error!Ast.Node.Index {
    ...
    if (function_type_def) |type_def| {
        self.globals.items[slot].type_def = self.gc.get(obj.ObjTypeDef, type_def);
    }
    ...
    if (!is_wasm) {
        for (zdefs) |zdef| {
            ...
            slot = try self.declareVariable(
                @intCast(node_slot),
                self.gc.get(obj.ObjTypeDef, zdef.type_def),
                zdef_name_token,
                true,
                false,
            );
...
fn userVarDeclaration(self: *Self, identifier: Ast.TokenIndex, final: bool, mutable: bool, omits_qualifier: bool) Error!Ast.Node.Index {
    ...
    if (self.resolveGeneric(try self.gc.copyString(self.ast.tokens.items(.lexeme)[self.current_token.? - 1]))) |generic_type| {
        ...
        if (self.current.?.generics.?.get((try self.gc.copyString(self.ast.tokens.items(.lexeme)[self.current_token.? - 1])).toIdx())) |generic_type| {
            var_type = self.gc.get(obj.ObjTypeDef, generic_type);
        }
    }
...
fn forEachStatement(self: *Self) Error!Ast.Node.Index {
    ...
    if (key_omitted) {
        ...
        key = try self.implicitVarDeclaration(
            ...
            self.gc.get(obj.ObjTypeDef, self.gc.type_registry.void_type),
            false,
            false,
        );
    ...
    const map_index = try self.implicitVarDeclaration(
        ...
        self.gc.get(obj.ObjTypeDef, self.gc.type_registry.double_type),
        false,
        false,
    );
    ...
    const key_type = switch (iterable_type_def.def_type) {
        .List, .String => self.gc.get(obj.ObjTypeDef, self.gc.type_registry.int_type),
        .Map => iterable_type_def.resolved_type.?.Map.key_type,
        .Range => self.gc.get(obj.ObjTypeDef, self.gc.type_registry.int_type),
    ...
    const value_type = switch (iterable_type_def.def_type) {
        ...
        .Range => self.gc.get(obj.ObjTypeDef, self.gc.type_registry.int_type),
        .String => self.gc.get(obj.ObjTypeDef, self.gc.type_registry.str_type),
        .Map => iterable_type_def.resolved_type.?.Map.value_type,
        .Enum => try iterable_type_def.toInstance(&self.gc.type_registry, false),
        .Fiber => self.gc.get(obj.ObjTypeDef, iterable_type_def.resolved_type.?.Fiber.yield_type),
        .Placeholder => placeholder: {
            const placeholder = try self.gc.type_registry.getTypeDef(
                .{
                    .def_type = .Placeholder,
    ...
    while (try self.match(.Catch)) {
        if (try self.match(.LeftParen)) {
            ...
            _ = try self.parseVariable(
                @intCast(node_slot),
                identifier,
                self.gc.get(obj.ObjTypeDef, self.ast.nodes.items(.type_def)[type_def].?),
                true, // function arguments are final
                false,
