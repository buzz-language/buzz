...
fn parseFunctionType(self: *Self, parent_generic_types: ?std.AutoArrayHashMap(*obj.ObjString, *obj.ObjTypeDef)) Error!Ast.Node.Index {
    ...
    const function_def: obj.ObjFunction.FunctionDef = .{
        ...
        .return_type = if (return_type) |rt|
            try self.ast.nodes.items(.type_def)[rt].?.toInstance(
                self.gc.allocator,
                &self.gc.type_registry,
                null,
            )
        ...
        .yield_type = if (yield_type) |yt|
            try self.ast.nodes.items(.type_def)[yt].?.toInstance(
                self.gc.allocator,
                &self.gc.type_registry,
                null,
            )
                ...
                if (common_type == null) {
                    ...
                    if (!common_type.?.eql(actual_type_def)) {
                        if (common_type.?.def_type == .ObjectInstance and actual_type_def.def_type == .ObjectInstance) {
                            common_type = common_type.?.resolved_type.?
                                .ObjectInstance.of.resolved_type.?
                                .Object.bothConforms(actual_type_def.resolved_type.?.ObjectInstance.of.resolved_type.?.Object) orelse
                                common_type;
                            common_type = try common_type.?.toInstance(
                                self.gc.allocator,
                                &self.gc.type_registry,
                                null,
                            );
                ...
                if (common_key_type == null) {
                    ...
                    if (!common_key_type.?.eql(actual_type_def)) {
                        if (common_key_type.?.def_type == .ObjectInstance and
                            actual_type_def.def_type == .ObjectInstance and
                            common_key_type.?.isMutable() == actual_type_def.isMutable())
                        {
                            common_key_type = common_key_type.?.resolved_type.?.ObjectInstance.of.resolved_type.?.Object.bothConforms(
                ...
                if (common_value_type == null) {
                    ...
                    if (!common_value_type.?.eql(actual_type_def)) {
                        if (common_value_type.?.def_type == .ObjectInstance and
                            actual_type_def.def_type == .ObjectInstance and
                            common_value_type.?.isMutable() == actual_type_def.isMutable())
                        {
                            common_value_type = common_value_type.?.resolved_type.?.ObjectInstance.of.resolved_type.?.Object.bothConforms(
                                actual_type_def.resolved_type.?.ObjectInstance.of.resolved_type.?.Object,
