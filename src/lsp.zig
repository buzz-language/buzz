...
const Document = struct {
                    ...
                    if (global.type_def.def_type == .Object and global.type_def.resolved_type != null) {
                        ...
                        while (it.next()) |kv| {
                            ...
                            try children.append(
                                ...
                                .{
                                    .name = field.name,
                                    .detail = try self.gc.get(o.ObjTypeDef, field.type_def).toStringAlloc(allocator, false, &self.gc),
                    ...
                    if (global.type_def.def_type == .Protocol and global.type_def.resolved_type != null) {
                        ...
                        while (it.next()) |kv| {
                            ...
                            try children.append(
                                ...
                                .{
                                    .name = kv.key_ptr.*,
                                    .detail = try self.gc.get(o.ObjTypeDef, method.type_def).toStringAlloc(allocator, false, &self.gc),
                                    .kind = .Method,
                                    .range = range,
                                    .selectionRange = range,
                                },
                    ...
                    try self.symbols.append(
                        ...
                        .{
                            .name = if (fun_def.function_type == .Test)
                                ...
                                self.gc.get(o.ObjString, fun_def.name).string,
                            .detail = try global.type_def.toStringAlloc(allocator, false, &self.gc),
                            .kind = .Function,
    ...
    fn builtinMemberDoc(self: *Document, node_index: Ast.Node.Index) ?[]const u8 {
        ...
        const callee_type = self.gc.get(o.ObjTypeDef, callee_type_def);
    ...
    fn dotMemberHoverInfo(self: *Document, node_index: Ast.Node.Index) !HoverInfo {
        ...
        return .{
            .type_def = if (self.ast.nodes.items(.type_def)[node_index]) |type_def| self.gc.get(o.ObjTypeDef, type_def) else null,
            .doc = doc,
        };
    }
    ...
    fn nodeHoverInfo(self: *Document, node_index: Ast.Node.Index) !?HoverInfo {
        ...
        return .{
            .type_def = self.gc.get(o.ObjTypeDef, type_def),
    ...
    fn objectInitFieldHoverInfo(self: *Document, node_index: Ast.Node.Index, token: Ast.TokenIndex) ?HoverInfo {
        ...
        const resolved_type = self.gc.get(o.ObjTypeDef, type_def);

        if (resolved_type.def_type != .ObjectInstance) {
            return null;
        }

        const object_def = self.gc.get(o.ObjTypeDef, resolved_type.resolved_type.?.ObjectInstance.of).resolved_type.?.Object;
        const field = object_def.fields.get(ast_slice.tokens.items(.lexeme)[token]) orelse return null;

        return .{
            .type_def = self.gc.get(o.ObjTypeDef, field.type_def),
                    ...
                    switch (callee_type.def_type) {
                        ...
                        .ObjectInstance, .Object => {
                            const object_def = if (callee_type.def_type == .ObjectInstance)
                                self.gc.get(o.ObjTypeDef, callee_type.resolved_type.?.ObjectInstance.of).resolved_type.?.Object
                            else
            ...
            .UserType => {
                if (ast_slice.nodes.items(.type_def)[node]) |type_def| {
                    ...
                    if (switch (resolved_type.def_type) {
                        ...
                        .ObjectInstance => self.gc.get(o.ObjTypeDef, resolved_type.resolved_type.?.ObjectInstance.of).resolved_type.?.Object.location,
                        .Enum => resolved_type.resolved_type.?.Enum.location,
                        .EnumInstance => self.gc.get(o.ObjTypeDef, resolved_type.resolved_type.?.EnumInstance.of).resolved_type.?.Enum.location,
                        .Protocol => resolved_type.resolved_type.?.Protocol.location,
                        .ProtocolInstance => self.gc.get(o.ObjTypeDef, resolved_type.resolved_type.?.ProtocolInstance.of).resolved_type.?.Protocol.location,
                        else => null,
                    }) |location| {
                        try self.definitions.put(
                            allocator,
            ...
            switch (ast.nodes.items(.tag)[node]) {
                .VarDeclaration => {
                    ...
                    if (!comp.implicit and comp.type == null and type_def != null) {
                        try self.addTypeInlay(
                            allocator,
                            self.document.gc.get(o.ObjTypeDef, type_def.?),
                            name,
                            ": ",
                            null,
                            0,
                ...
                .Function => {
                    ...
                    if (comp.test_message == null and fun_type.lambda and fun_type.return_type == null and fun_type_def != null) {
                        try self.addTypeInlay(
                            allocator,
                            self.document.gc.get(o.ObjTypeDef, self.document.gc.get(o.ObjTypeDef, fun_type_def.?).resolved_type.?.Function.return_type),
                            location,
                            " > ",
                            null,
                            0,
                    ...
                    for (comp.properties) |property| {
                        const prop_type = if (comp.object) |object|
                            if (type_defs[object]) |type_def|
                                if (self.document.gc.get(o.ObjTypeDef, type_def).def_type == .Object)
                                    if (self.document.gc.get(o.ObjTypeDef, type_def).resolved_type.?.Object.fields.get(lexemes[property.name])) |field|
                                        field.type_def
                                    else
                                        null
                                else
        ...
        pub fn processNode(
            ...
            switch (ast.nodes.items(.tag)[node]) {
                .ObjectDeclaration => {
                    ...
                    const resolved_type = self.document.gc.get(o.ObjTypeDef, type_def);
    ...
    fn dotCompletion(
        ...
        if (dot_callee) |callee| {
            if (document.ast.nodes.items(.type_def)[callee]) |callee_type_def| {
                const callee_type = document.gc.get(o.ObjTypeDef, callee_type_def);
                ...
                switch (callee_type.def_type) {
                    ...
                    .ObjectInstance => {
                        var it = document.gc.get(o.ObjTypeDef, callee_type.resolved_type.?.ObjectInstance.of).resolved_type.?.Object.fields.iterator();
                        while (it.next()) |kv| {
                            if (!kv.value_ptr.*.static) {
                                try appendCompletionItem(
                                    result,
                    ...
                    .ProtocolInstance => {
                        for (document.gc.get(o.ObjTypeDef, callee_type.resolved_type.?.ProtocolInstance.of).resolved_type.?.Protocol.methods.keys()) |key| {
                            try appendCompletionItem(
                                result,
                                allocator,
                                key,
