...
pub const Renderer = struct {
    ...
    fn stringEscape(
        ...
        for (self.gc.get(obj.ObjString, literal.literal).string) |byte| switch (byte) {
            '{' => try writer.writeAll("\\{"),
    ...
    fn renderAnonymousObjectType(self: *Self, node: Ast.Node.Index, space: Space) Error!void {
        ...
        // }
        try self.renderExpectedToken(
            if (self.gc.get(obj.ObjTypeDef, object_type).optional) end_location - 1 else end_location,
            .RightBrace,
            if (self.gc.get(obj.ObjTypeDef, object_type).optional) .None else space,
        );

        // ?
        if (self.gc.get(obj.ObjTypeDef, object_type).optional) {
            try self.renderExpectedToken(
                end_location,
                .Question,
                space,
    ...
    fn renderCall(self: *Self, node: Ast.Node.Index, space: Space) Error!void {
        ...
        const opening_tag = self.ast.tokens.items(.tag)[token];
        const width_multiline = try self.nodeWouldExceedLine(node);
        ...
        if (!invoked) {
            try self.renderNode(
                comp.callee,
                if (opening_tag == .Dot) .Space else .None,
            );
        }
        ...
        if (opening_tag == .Dot) {
            ...
            if (comp.catch_default) |catch_default| {
                try self.renderExpectedToken(
                    end_locations[comp.arguments[0].value] + 1,
                    .Catch,
                    .Space,
                );
        ...
        if (comp.catch_default) |catch_default| {
            // catch
            try self.renderExpectedToken(token + 1, .Catch, .Space);
    ...
    fn renderMatch(self: *Self, node: Ast.Node.Index, space: Space) Error!void {
        ...
        for (components.branches, 0..) |branch, bidx| {
            const last_token = end_locations[branch.conditions[branch.conditions.len - 1]] + 1;
            const conditions_ends_with_comma = tags[last_token] == .Comma;
            ...
            // ->
            try self.renderExpectedToken(
                if (conditions_ends_with_comma) last_token + 1 else last_token,
                .Arrow,
                .Space,
            );
    ...
    fn renderObjectDeclaration(self: *Self, node: Ast.Node.Index, _: Space) Error!void {
        ...
        const fields = self.gc.get(obj.ObjTypeDef, type_defs[node].?).resolved_type.?
            .Object.fields;
        ...
        for (components.members, 0..) |member, i| {
            ...
            if (member.method) {
                ...
                try self.renderNode(
                    ...
                    if (self.gc.get(obj.ObjTypeDef, type_defs[member.method_or_default_value.?].?).resolved_type.?.Function.lambda)
                        .Semicolon
                    else
                        .Newline,
                );
                ...
                const has_default_value = if (member.method_or_default_value) |default_value|
                    !self.gc.get(obj.ObjTypeDef, type_defs[member.property_type.?].?).optional or self.ast.nodes.items(.tag)[default_value] != .Null
                else
                    false;
    ...
    fn renderProtocolDeclaration(self: *Self, node: Ast.Node.Index, _: Space) Error!void {
        ...
        for (components.methods, 0..) |method, i| {
            ...
            // mut
            if (self.gc.get(obj.ObjTypeDef, type_defs[node].?).resolved_type.?
                .Protocol.methods
                .get(
                    self.gc.get(obj.ObjString, self.gc.get(obj.ObjTypeDef, type_defs[method.method].?).resolved_type.?.Function.name).string,
                ).?.mutable)
            {
                try self.renderExpectedToken(
