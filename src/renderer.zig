    ...
    return struct {
        ...
        fn renderBlock(self: *Self, node: Ast.Node.Index, space: Space) Error!void {
            ...
            for (statements, 0..) |stmt, i| {
                ...
                try self.ais.pushSpace(.Semicolon);
        ...
        fn stringEscape(
            ...
            for (bytes) |byte| switch (byte) {
                '{' => try writer.writeAll("\\{"),
                '\n' => if (f.len == 1 and f[0] == '`') try writer.writeAll("\n") else try writer.writeAll("\\n"),
                '\r' => if (f.len == 1 and f[0] == '`') try writer.writeAll("\r") else try writer.writeAll("\\r"),
                '\t' => if (f.len == 1 and f[0] == '`') try writer.writeAll("\t") else try writer.writeAll("\\t"),
                '\\' => try writer.writeAll("\\\\"),
                '"' => {
                    if (f.len == 1 and f[0] == '`') {
        ...
        fn renderStringLiteral(self: *Self, node: Ast.Node.Index, space: Space) Error!void {
            ...
            var formatter = std.fmt.Formatter(stringEscape){
                .data = string_literal.literal.string,
            };
            ...
            if (string_literal.delimiter == '"')
                ...
                // Possibly multilined string, we don't touch indenting or spaces
                try self.ais.writeFixingWhitespace(escaped.items);
            }
