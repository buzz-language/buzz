        ...
        .Object => object: {
            ...
            if (expected_object.anonymous or actual_object.anonymous) {
                ...
                while (it.next()) |entry| {
                    ...
                    if (!typeCompatible(
                        gc.get(o.ObjTypeDef, entry.value_ptr.type_def),
                        gc.get(o.ObjTypeDef, other_field.type_def),
                        gc,
                    )) {
                        break :object false;
                    }
            ...
            for (expected_generics, actual_generics) |expected_generic, actual_generic| {
                if (!typeCompatible(gc.get(o.ObjTypeDef, expected_generic), gc.get(o.ObjTypeDef, actual_generic), gc)) {
                    break :object false;
                }
            }
...
fn typeCompatible(expected: *o.ObjTypeDef, actual: *o.ObjTypeDef, gc: *GC) bool {
    ...
    const type_match = switch (expected.def_type) {
        ...
        .Fiber => actual.def_type == .Fiber and
            typeCompatible(
                gc.get(o.ObjTypeDef, expected.resolved_type.?.Fiber.return_type),
                gc.get(o.ObjTypeDef, actual.resolved_type.?.Fiber.return_type),
                gc,
            ) and
            typeCompatible(
                gc.get(o.ObjTypeDef, expected.resolved_type.?.Fiber.yield_type),
                gc.get(o.ObjTypeDef, actual.resolved_type.?.Fiber.yield_type),
                gc,
            ),
        ...
        .ObjectInstance => actual.def_type == .ObjectInstance and
            namedTypeCompatible(
                gc.get(o.ObjTypeDef, expected.resolved_type.?.ObjectInstance.of),
                gc.get(o.ObjTypeDef, actual.resolved_type.?.ObjectInstance.of),
                gc,
            ),
        ...
        .ProtocolInstance => switch (actual.def_type) {
            .ProtocolInstance => namedTypeCompatible(
                gc.get(o.ObjTypeDef, expected.resolved_type.?.ProtocolInstance.of),
                gc.get(o.ObjTypeDef, actual.resolved_type.?.ProtocolInstance.of),
                gc,
            ),
            .ObjectInstance => object: {
                const conforms_to = gc.get(
                    o.ObjTypeDef,
                    actual.resolved_type.?.ObjectInstance.of,
                ).resolved_type.?.Object.conforms_to;
                var it = conforms_to.iterator();
                while (it.next()) |entry| {
                    if (namedTypeCompatible(
                        gc.get(o.ObjTypeDef, expected.resolved_type.?.ProtocolInstance.of),
                        gc.get(o.ObjTypeDef, entry.key_ptr.*),
                        gc,
                    )) {
                        break :object true;
                    }
        ...
        .EnumInstance => actual.def_type == .EnumInstance and
            namedTypeCompatible(
                gc.get(o.ObjTypeDef, expected.resolved_type.?.EnumInstance.of),
                gc.get(o.ObjTypeDef, actual.resolved_type.?.EnumInstance.of),
                gc,
            ),
        ...
        .Function => function: {
            ...
            if (!typeCompatible(
                gc.get(o.ObjTypeDef, expected_function.return_type),
                gc.get(o.ObjTypeDef, actual_function.return_type),
                gc,
            ) or
                !typeCompatible(
                    gc.get(o.ObjTypeDef, expected_function.yield_type),
                    gc.get(o.ObjTypeDef, actual_function.yield_type),
                    gc,
                ) or
            ...
            for (expected_keys, 0..) |expected_key, index| {
                if (!typeCompatible(
                    gc.get(o.ObjTypeDef, expected_function.parameters.get(expected_key).?),
                    gc.get(o.ObjTypeDef, actual_function.parameters.get(actual_keys[index]).?),
                    gc,
                )) {
                    break :function false;
                }
...
fn inferListType(ast: Ast.Slice, reporter: *Reporter, gc: *GC, value: Ast.Node.Index, target_type: *o.ObjTypeDef) error{OutOfMemory}!bool {
    ...
    if (list.explicit_item_type == null and (list.items.len == 0 or inferred_item)) {
        type_defs[value] = (target_type.cloneMutable(
            &gc.type_registry,
            gc.get(o.ObjTypeDef, type_defs[value].?).isMutable(),
...
fn matchTypeFromBranches(ast: Ast.Slice, gc: *GC, match: Ast.Node.Index) error{OutOfMemory}!?*o.ObjTypeDef {
    ...
    for (components.branches) |branch| {
        if (type_defs[branch.expression]) |branch_type_def| {
            const resolved_branch_type_def = gc.get(o.ObjTypeDef, branch_type_def);
            ...
            if (resolved_branch_type_def.def_type == .Void) {
                ...
                if (previous.def_type == .Void) {
                    ...
                    type_def = gc.get(o.ObjTypeDef, gc.type_registry.any_type);
                    break;
                }
            } else {
                type_def = resolved_branch_type_def;
    ...
    if (components.else_branch) |else_branch| {
        if (type_defs[else_branch]) |else_type_def| {
            const resolved_else_type_def = gc.get(o.ObjTypeDef, else_type_def);
...
fn populateAnonymousEnumCase(ast: Ast.Slice, reporter: *Reporter, gc: *GC, value: Ast.Node.Index, target_type: *o.ObjTypeDef) bool {
    ...
    if (target_type.def_type == .EnumInstance) {
        ...
        const enum_type_def = gc.get(o.ObjTypeDef, target_type.resolved_type.?.EnumInstance.of).resolved_type.?.Enum;
    ...
    if (gc.get(o.ObjTypeDef, type_defs[value].?).def_type == .Placeholder) {
        const locations = ast.nodes.items(.location);
        const end_locations = ast.nodes.items(.end_location);
...
fn populateAnonymousObject(ast: Ast.Slice, reporter: *Reporter, gc: *GC, value: Ast.Node.Index, target_type: *o.ObjTypeDef) error{OutOfMemory}!bool {
    ...
    if (target_type.def_type != .ObjectInstance or
        ...
        gc.get(o.ObjTypeDef, type_defs[value].?).def_type != .ObjectInstance)
    {
        return false;
    }

    const anon_object_type = gc.get(o.ObjTypeDef, type_defs[value].?).resolved_type.?.ObjectInstance.of;
    const target_object_type = target_type.resolved_type.?.ObjectInstance.of;
    if (gc.get(o.ObjTypeDef, anon_object_type) == gc.get(o.ObjTypeDef, target_object_type)) {
    ...
    if (!gc.get(o.ObjTypeDef, anon_object_type).resolved_type.?.Object.anonymous) {
        return false;
    }

    var anon_fields = &gc.get(o.ObjTypeDef, anon_object_type).resolved_type.?.Object.fields;
    ...
    const target_fields = gc.get(o.ObjTypeDef, target_object_type).resolved_type.?.Object
        .fields;
    ...
    for (object_init.properties) |property| {
        ...
        _ = try inferType(ast, reporter, gc, property.value, gc.get(o.ObjTypeDef, target_field.type_def));

        const value_type_def = gc.get(o.ObjTypeDef, type_defs[property.value] orelse return false);
        anon_field.type_def = value_type_def.toIdx();

        if (!typeCompatible(gc.get(o.ObjTypeDef, target_field.type_def), value_type_def, gc)) {
            return false;
        }
    }
...
fn checkBinary(ast: Ast.Slice, reporter: *Reporter, gc: *GC, _: ?Ast.Node.Index, node: Ast.Node.Index) error{OutOfMemory}!bool {
    ...
    var left_type = nodeType(ast, gc, node_components.Binary.left) orelse gc.get(
        o.ObjTypeDef,
        gc.type_registry.any_type,
    );
    var right_type = nodeType(ast, gc, node_components.Binary.right) orelse gc.get(
        o.ObjTypeDef,
        gc.type_registry.any_type,
    );
    ...
    if (node_components.Binary.operator == .QuestionQuestion and left_type.def_type != .Placeholder) {
        ...
        if (try inferType(ast, reporter, gc, node_components.Binary.right, fallback_type)) {
            right_type = nodeType(ast, gc, node_components.Binary.right) orelse gc.get(
                o.ObjTypeDef,
                gc.type_registry.any_type,
            );
        }
        ...
        right_type.def_type != .Placeholder)
    {
        ...
        if (try inferType(ast, reporter, gc, node_components.Binary.left, right_type)) {
            left_type = nodeType(ast, gc, node_components.Binary.left) orelse gc.get(
                o.ObjTypeDef,
                gc.type_registry.any_type,
            );
        }
    ...
    switch (node_components.Binary.operator) {
        ...
        => {
            if (!typeCompatible(left_type, right_type, gc)) {
                reporter.reportTypeCheckWithGc(
                    gc,
                    .binary_operand_type,
...
fn checkCall(ast: Ast.Slice, reporter: *Reporter, gc: *GC, _: ?Ast.Node.Index, node: Ast.Node.Index) error{OutOfMemory}!bool {
    ...
    const callee_type = switch (ast.nodes.items(.tag)[components.callee]) {
        .Dot => gc.get(o.ObjTypeDef, node_components[components.callee].Dot.member_type_def),
        else => gc.get(o.ObjTypeDef, type_defs[components.callee].?),
    };
    ...
    for (components.arguments, 0..) |argument, index| {
        ...
        var argument_type_def = gc.get(o.ObjTypeDef, type_defs[argument.value].?);
        ...
        if (def_arg_type) |arg_type| {
            ...
            _ = try inferType(ast, reporter, gc, argument.value, gc.get(o.ObjTypeDef, arg_type));
            argument_type_def = gc.get(o.ObjTypeDef, type_defs[argument.value].?);

            if (!typeCompatible(gc.get(o.ObjTypeDef, arg_type), argument_type_def, gc)) {
                reporter.reportTypeCheckWithGc(
                    gc,
                    .call_argument_type,
                    ast.tokens.get(locations[components.callee]),
                    ast.tokens.get(end_locations[components.callee]),
                    gc.get(o.ObjTypeDef, arg_type),
            ...
            _ = missing_arguments.orderedRemove(gc.get(o.ObjString, actual_arg_key).string);
        } else {
            reporter.reportErrorFmt(
                .call_arguments,
                ast.tokens.get(locations[argument.value]),
...
fn checkDot(ast: Ast.Slice, reporter: *Reporter, gc: *GC, _: ?Ast.Node.Index, node: Ast.Node.Index) error{OutOfMemory}!bool {
    ...
    const callee_type = nodeType(ast, gc, components.callee) orelse gc.get(o.ObjTypeDef, gc.type_registry.any_type);
    ...
    if (tags[components.identifier] == .IntegerValue and
        (callee_type.def_type != .ObjectInstance or
            !gc.get(o.ObjTypeDef, callee_type.resolved_type.?.ObjectInstance.of)
            ...
            switch (components.member_kind) {
                .Value => {
                    ...
                    switch (callee_type.def_type) {
                        .ForeignContainer => {
                            if (callee_type.resolved_type.?.ForeignContainer.buzz_type.get(field_name)) |field_type| {
                                if (!typeCompatible(gc.get(o.ObjTypeDef, field_type), value_type_def, gc)) {
                                    reporter.reportTypeCheckWithGc(
                                        gc,
                                        .assignment_value_type,
                                        ast.tokens.get(callee_type.resolved_type.?.ForeignContainer.location),
                                        ast.tokens.get(callee_type.resolved_type.?.ForeignContainer.location),
                                        gc.get(o.ObjTypeDef, field_type),
                                        ast.tokens.get(locations[value]),
                                        ast.tokens.get(end_locations[value]),
                                        value_type_def,
                                        "Bad property type",
                        ...
                        .ObjectInstance, .Object => {
                            ...
                            _ = try inferType(ast, reporter, gc, value, gc.get(o.ObjTypeDef, field.?.type_def));
                            value_type_def = gc.get(o.ObjTypeDef, type_defs[value].?);

                            if (!typeCompatible(gc.get(o.ObjTypeDef, field.?.type_def), value_type_def, gc)) {
                                reporter.reportTypeCheckWithGc(
                                    gc,
                                    .assignment_value_type,
                                    ast.tokens.get(field.?.location),
                                    ast.tokens.get(field.?.location),
                                    gc.get(o.ObjTypeDef, field.?.type_def),
                                    ast.tokens.get(locations[value]),
                                    ast.tokens.get(end_locations[value]),
                                    value_type_def,
                                    "Bad property type",
                    ...
                    switch (tags[assign_token]) {
                        .PlusEqual => switch (gc.get(o.ObjTypeDef, type_defs[value].?).def_type) {
                            .Integer,
                            .Double,
                            .List,
                            .Map,
                        ...
                        .SlashEqual,
                        .PercentEqual,
                        => switch (gc.get(o.ObjTypeDef, type_defs[value].?).def_type) {
                            .Integer, .Double => {},
                            else => {
                                reporter.report(
                                    .arithmetic_operand_type,
                        ...
                        .BnotEqual,
                        .AmpersandEqual,
                        => if (gc.get(o.ObjTypeDef, type_defs[value].?).def_type != .Integer) {
                            reporter.report(
                                .arithmetic_operand_type,
                                ast.tokens.get(assign_token),
                                ast.tokens.get(assign_token),
...
fn checkDoUntil(ast: Ast.Slice, reporter: *Reporter, gc: *GC, _: ?Ast.Node.Index, node: Ast.Node.Index) error{OutOfMemory}!bool {
    ...
    const condition_type_def = nodeType(ast, gc, components.condition) orelse gc.get(
        o.ObjTypeDef,
        gc.type_registry.any_type,
    );
...
fn checkEnum(ast: Ast.Slice, reporter: *Reporter, gc: *GC, _: ?Ast.Node.Index, node: Ast.Node.Index) error{OutOfMemory}!bool {
    ...
    const enum_type = gc.get(o.ObjTypeDef, gc.get(o.ObjTypeDef, type_defs[node].?).resolved_type.?.Enum.enum_type);
    ...
    for (components.cases) |case| {
        const case_type_def = if (case.value) |value|
            gc.get(o.ObjTypeDef, type_defs[value].?)
        else
            null;
...
fn checkForEach(ast: Ast.Slice, reporter: *Reporter, gc: *GC, _: ?Ast.Node.Index, node: Ast.Node.Index) error{OutOfMemory}!bool {
    ...
    const iterable_type_def = nodeType(ast, gc, components.iterable) orelse gc.get(
        o.ObjTypeDef,
        gc.type_registry.any_type,
    );
    var key_type_def = nodeType(ast, gc, components.key) orelse gc.get(o.ObjTypeDef, gc.type_registry.any_type);
    const value_type_def = nodeType(ast, gc, components.value) orelse gc.get(o.ObjTypeDef, gc.type_registry.any_type);
    ...
    switch (iterable_type_def.def_type) {
        ...
        .Fiber => {
            const iterable_type = gc.get(o.ObjTypeDef, iterable_type_def.resolved_type.?.Fiber.yield_type).toInstance(
                &gc.type_registry,
                false,
            ) catch return error.OutOfMemory;
            if (!iterable_type.strictEql(value_type_def)) {
...
fn checkReturnValue(
    ...
    const locations = ast.nodes.items(.location);
    const end_locations = ast.nodes.items(.end_location);
    const current_function_type_def = gc.get(o.ObjTypeDef, type_defs[function_node].?).resolved_type.?.Function;

    if (value) |value_node| {
        // Function return types provide context for inferred return values.
        _ = try inferType(ast, reporter, gc, value_node, gc.get(o.ObjTypeDef, current_function_type_def.return_type));
        ...
        if (!gc.get(o.ObjTypeDef, current_function_type_def.return_type).eql(value_type_def.?)) {
            reporter.reportTypeCheckWithGc(
                gc,
                .return_type,
    ...
    } else if (gc.get(o.ObjTypeDef, current_function_type_def.return_type).def_type != .Void) {
        reporter.reportTypeCheckWithGc(
            gc,
            .return_type,
            ast.tokens.get(locations[function_node]),
...
fn checkFunction(ast: Ast.Slice, reporter: *Reporter, gc: *GC, _: ?Ast.Node.Index, node: Ast.Node.Index) error{OutOfMemory}!bool {
    ...
    if (function_signature) |signature| {
        for (signature.arguments) |argument| {
            ...
            _ = try inferType(ast, reporter, gc, default, gc.get(o.ObjTypeDef, param));
            const default_type_def = type_defs[default].?;

            if (gc.get(o.ObjTypeDef, default_type_def).isMutable()) {
                reporter.reportErrorAt(
                    .constant_default,
            ...
            if (!gc.get(o.ObjTypeDef, param).eql(gc.get(o.ObjTypeDef, default_type_def))) {
                reporter.reportTypeCheckWithGc(
                    gc,
                    .default_value_type,
                    ast.tokens.get(locations[argument.type]),
                    ast.tokens.get(end_locations[argument.type]),
                    gc.get(o.ObjTypeDef, param),
                    ast.tokens.get(locations[default]),
                    ast.tokens.get(end_locations[default]),
                    gc.get(o.ObjTypeDef, default_type_def),
    ...
    if (function_def.lambda and gc.get(o.ObjTypeDef, function_def.return_type).def_type != .Void) {
...
fn checkIf(ast: Ast.Slice, reporter: *Reporter, gc: *GC, _: ?Ast.Node.Index, node: Ast.Node.Index) error{OutOfMemory}!bool {
    ...
    if (!components.is_statement) {
        ...
        if (!gc.get(o.ObjTypeDef, type_defs[node].?).eql(gc.get(o.ObjTypeDef, type_defs[components.body].?))) {
            reporter.reportTypeCheckWithGc(
                gc,
                .inline_if_body_type,
                ast.tokens.get(location),
                ast.tokens.get(end_locations[node]),
                gc.get(o.ObjTypeDef, type_defs[node].?),
                ast.tokens.get(locations[components.body]),
                ast.tokens.get(end_locations[components.body]),
                gc.get(o.ObjTypeDef, type_defs[components.body].?),
                "Inline if body type not matching",
            );
            had_error = true;
        }

        if (!gc.get(o.ObjTypeDef, type_defs[node].?).eql(gc.get(o.ObjTypeDef, type_defs[components.else_branch.?].?))) {
            reporter.reportTypeCheckWithGc(
                gc,
                .inline_if_else_type,
                ast.tokens.get(location),
                ...
                gc.get(o.ObjTypeDef, type_defs[components.else_branch.?].?),
    ...
    if (components.casted_type == null and components.unwrapped_identifier == null) {
        if (gc.get(o.ObjTypeDef, type_defs[components.condition].?).def_type != .Boolean) {
            reporter.reportErrorAt(
                .if_condition_type,
                ast.tokens.get(locations[components.condition]),
                ast.tokens.get(end_locations[components.condition]),
...
fn checkMatch(ast: Ast.Slice, reporter: *Reporter, gc: *GC, _: ?Ast.Node.Index, node: Ast.Node.Index) error{OutOfMemory}!bool {
    ...
    const value_type_def = gc.get(o.ObjTypeDef, type_defs[node_components.value].?);
        ...
        switch (value_type_def.def_type) {
            ...
            .String => for (node_components.branches) |branch| {
                for (branch.conditions) |condition| {
                    ...
                    if (condition_type_def == null or gc.get(o.ObjTypeDef, condition_type_def.?).optional or
                        (gc.get(o.ObjTypeDef, condition_type_def.?).def_type != .String and
                            gc.get(o.ObjTypeDef, condition_type_def.?).def_type != .Pattern and
                            gc.get(o.ObjTypeDef, condition_type_def.?).def_type != .Type))
                    {
                        reporter.reportErrorAt(
            ...
            .Integer, .Double => for (node_components.branches) |branch| {
                for (branch.conditions) |condition| {
                    const condition_type_def = type_defs[condition];
                    if (condition_type_def == null or gc.get(o.ObjTypeDef, condition_type_def.?).optional or
                        (gc.get(o.ObjTypeDef, condition_type_def.?).def_type != .Integer and
                            gc.get(o.ObjTypeDef, condition_type_def.?).def_type != .Range and
                            gc.get(o.ObjTypeDef, condition_type_def.?).def_type != .Double and
                            gc.get(o.ObjTypeDef, condition_type_def.?).def_type != .Type))
                    {
                        reporter.reportErrorAt(
                            .match_condition_type,
            ...
            .Type, .Any => {},
                ...
                for (branch.conditions) |condition| {
                    ...
                    if (condition_type_def == null or
                        (!value_type_def.eql(gc.get(o.ObjTypeDef, condition_type_def.?)) and
                            gc.get(o.ObjTypeDef, condition_type_def.?).def_type != .Type))
                    {
                        reporter.reportTypeCheckWithGc(
                            gc,
                            ...
                            ast.tokens.get(locations[condition]),
                            ast.tokens.get(end_locations[condition]),
                            gc.get(o.ObjTypeDef, condition_type_def.?),
            ...
            for (branch.conditions) |condition| {
                ...
                for (seen_values.items, seen_conditions.items) |seen_value, seen_condition| {
                    ...
                    if (!duplicate_condition and numeric_match_value) {
                        ...
                        const condition_is_range = !gc.get(o.ObjTypeDef, condition_type_def).optional and
                            gc.get(o.ObjTypeDef, condition_type_def).def_type == .Range;
                        const seen_condition_is_range = !gc.get(o.ObjTypeDef, seen_condition_type_def).optional and
                            gc.get(o.ObjTypeDef, seen_condition_type_def).def_type == .Range;
...
fn checkList(ast: Ast.Slice, reporter: *Reporter, gc: *GC, _: ?Ast.Node.Index, node: Ast.Node.Index) error{OutOfMemory}!bool {
    ...
    const type_defs = ast.nodes.items(.type_def);
    const item_type = gc.get(o.ObjTypeDef, type_defs[node].?).resolved_type.?.List.item_type;

    var had_error = false;

    for (components.items) |item| {
        if (item_type.def_type == .Placeholder) {
            reporter.reportPlaceholder(ast, gc, gc.get(o.ObjTypeDef, type_defs[item].?).resolved_type.?.Placeholder);
        } else {
...
fn checkMap(ast: Ast.Slice, reporter: *Reporter, gc: *GC, _: ?Ast.Node.Index, node: Ast.Node.Index) error{OutOfMemory}!bool {
    ...
    const map_type = gc.get(o.ObjTypeDef, type_defs[node].?).resolved_type.?.Map;
    ...
    for (components.entries) |entry| {
        ...
        if (!typeCompatible(key_type, gc.get(o.ObjTypeDef, type_defs[entry.key].?), gc)) {
            reporter.reportTypeCheckWithGc(
                gc,
                .map_key_type,
                ast.tokens.get(locations[node]),
                ...
                gc.get(o.ObjTypeDef, type_defs[entry.key].?),
                "Bad key type",
            );
            had_error = true;
        }
        ...
        if (!typeCompatible(value_type, gc.get(o.ObjTypeDef, type_defs[entry.value].?), gc)) {
            reporter.reportTypeCheckWithGc(
                gc,
                .map_value_type,
                ast.tokens.get(locations[node]),
                ...
                gc.get(o.ObjTypeDef, type_defs[entry.value].?),
                "Bad value type",
            );
            had_error = true;
        }
...
fn checkNamedVariable(ast: Ast.Slice, reporter: *Reporter, gc: *GC, _: ?Ast.Node.Index, node: Ast.Node.Index) error{OutOfMemory}!bool {
    ...
    if (components.value) |value| {
        ...
        _ = try inferType(ast, reporter, gc, value, gc.get(o.ObjTypeDef, type_defs[node].?));
        const value_type_def = gc.get(o.ObjTypeDef, type_defs[value].?);

        if (!typeCompatible(gc.get(o.ObjTypeDef, type_defs[node].?), value_type_def, gc)) {
            reporter.reportTypeCheckWithGc(
                gc,
                .assignment_value_type,
                ast.tokens.get(locations[node]),
        ...
        switch (tags[components.assign_token.?]) {
            .PlusEqual => switch (gc.get(o.ObjTypeDef, type_defs[node].?).def_type) {
                .Integer,
                .Double,
                .List,
                .Map,
            ...
            .SlashEqual,
            .PercentEqual,
            => switch (gc.get(o.ObjTypeDef, type_defs[node].?).def_type) {
                .Integer, .Double => {},
                else => {
                    reporter.report(
                        .arithmetic_operand_type,
            ...
            .BnotEqual,
            .AmpersandEqual,
            => if (gc.get(o.ObjTypeDef, type_defs[node].?).def_type != .Integer) {
                reporter.report(
                    .arithmetic_operand_type,
                    ast.tokens.get(components.assign_token.?),
                    ast.tokens.get(components.assign_token.?),
        ...
        while (method_it.next()) |mkv| {
            ...
            for (components.members) |member| {
                if (member.method and std.mem.eql(u8, ast.tokens.items(.lexeme)[member.name], mkv.key_ptr.*)) {
                    found = true;
                    const expected_type = gc.get(o.ObjTypeDef, mkv.value_ptr.*.type_def);
                    const actual_type = gc.get(o.ObjTypeDef, type_defs[member.method_or_default_value.?].?);
...
fn checkObjectInit(ast: Ast.Slice, reporter: *Reporter, gc: *GC, _: ?Ast.Node.Index, node: Ast.Node.Index) error{OutOfMemory}!bool {
    ...
    if (node_type_def.def_type == .ObjectInstance) {
        ...
        while (it.next()) |kv| {
            try fields.put(
                gc.allocator,
                kv.value_ptr.*.name,
                gc.get(o.ObjTypeDef, kv.value_ptr.*.type_def),
            );
        }
    } else {
        ...
        while (it.next()) |kv| {
            try fields.put(
                gc.allocator,
                kv.key_ptr.*,
                gc.get(o.ObjTypeDef, kv.value_ptr.*),
            );
        }
    }
    ...
    if (node_type_def.def_type != .ForeignContainer or node_type_def.resolved_type.?.ForeignContainer.zig_type != .Union or init_properties.count() == 0) {
        const field_defs = if (node_type_def.def_type == .ObjectInstance)
            gc.get(o.ObjTypeDef, node_type_def.resolved_type.?.ObjectInstance.of).resolved_type.?.Object.fields
        else
            null;
...
fn checkRange(ast: Ast.Slice, reporter: *Reporter, gc: *GC, _: ?Ast.Node.Index, node: Ast.Node.Index) error{OutOfMemory}!bool {
    ...
    if (gc.get(o.ObjTypeDef, type_defs[components.low].?).def_type != .Integer) {
        reporter.reportTypeCheckWithGc(
            gc,
            .range_type,
            null,
            ...
            gc.get(o.ObjTypeDef, type_defs[components.low].?),
            "Bad low range limit type",
        );
        had_error = true;
    }

    if (gc.get(o.ObjTypeDef, type_defs[components.high].?).def_type != .Integer) {
        reporter.reportTypeCheckWithGc(
            gc,
            .range_type,
            null,
            ...
            gc.get(o.ObjTypeDef, type_defs[components.high].?),
...
fn checkSubscript(ast: Ast.Slice, reporter: *Reporter, gc: *GC, _: ?Ast.Node.Index, node: Ast.Node.Index) error{OutOfMemory}!bool {
    ...
    var value_type_def = if (components.value) |value| gc.get(o.ObjTypeDef, type_defs[value].?) else null;
    ...
    switch (subscripted_type_def.def_type) {
        ...
        .List => {
            ...
            if (components.value) |value| {
                if (!gc.get(o.ObjTypeDef, type_defs[components.subscripted].?).isMutable()) {
                    const callee_type_str = gc.get(
                        o.ObjTypeDef,
                        type_defs[components.subscripted].?,
                    ).toStringAlloc(gc.allocator, false, gc) catch unreachable;
...
fn checkVarDeclaration(ast: Ast.Slice, reporter: *Reporter, gc: *GC, _: ?Ast.Node.Index, node: Ast.Node.Index) error{OutOfMemory}!bool {
    ...
    if (components.value) |value| {
        ...
        if (!typeCompatible(expected_type, gc.get(o.ObjTypeDef, value_type_def.?), gc) and
            !typeCompatible(expected_non_optional, gc.get(o.ObjTypeDef, value_type_def.?), gc))
        {
            reporter.reportTypeCheckWithGc(
                gc,
                .assignment_value_type,
                ...
                ast.tokens.get(locations[value]),
                ast.tokens.get(end_locations[value]),
                gc.get(o.ObjTypeDef, value_type_def.?),
                "Wrong variable type",
            );
...
fn checkYield(ast: Ast.Slice, reporter: *Reporter, gc: *GC, current_function_node: ?Ast.Node.Index, node: Ast.Node.Index) error{OutOfMemory}!bool {
    ...
    const current_function_typedef = gc.get(
        o.ObjTypeDef,
        type_defs[current_function_node.?].?,
    ...
    _ = try inferType(ast, reporter, gc, components, gc.get(o.ObjTypeDef, current_function_typedef.yield_type));
    ...
    if (type_def == null) {
        ...
        had_error = true;
    } else if (!gc.get(o.ObjTypeDef, current_function_typedef.yield_type).eql(gc.get(o.ObjTypeDef, type_def.?))) {
        reporter.reportTypeCheckWithGc(
            gc,
            .yield_type,
