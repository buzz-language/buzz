...
pub fn compileFunctionIfNeeded(self: *Self, closure: *o.ObjClosure) StartError!bool {
    ...
    const function = self.gc.get(o.ObjFunction, closure.function);

    switch (self.gc.get(o.ObjTypeDef, function.type_def).resolved_type.?.Function.function_type) {
        .Extern,
        .Script,
        .ScriptEntryPoint,
        .EntryPoint,
...
pub fn compile(self: *Self, ast: Ast.Slice, closure: *o.ObjClosure, hotspot_node: ?Ast.Node.Index) StartError!void {
    const ast_node = hotspot_node orelse self.gc.get(o.ObjFunction, closure.function).node;
...
pub fn compileFunctionSynchronously(self: *Self, closure: *o.ObjClosure) Error!void {
    ...
    if (self.gc.get(o.ObjFunction, closure.function).native_raw != null) {
        return;
    }
...
fn work(self: *Self) Error!void {
    ...
    while (self.jobs.front()) |job| {
        ...
        if (BuildOptions.jit_debug) {
            const function = self.gc.get(o.ObjFunction, job.closure.function);
            const function_type = self.gc.get(o.ObjTypeDef, function.type_def);
            const function_name = self.gc.get(o.ObjString, function_type.resolved_type.?.Function.name).string;
...
fn queueObjectMethodCollateral(self: *Self, object_type: *o.ObjTypeDef, method_index: usize) Error!void {
    for (self.state.?.closure.globals.items) |global| {
        ...
        if (self.gc.get(o.ObjTypeDef, object.type_def) != object_type) {
            continue;
        }
        ...
        try self.queueCollateralFunction(self.gc.get(o.ObjFunction, closure.function).node, closure);
...
fn getString(self: *Self, string: []const u8) Error!*o.ObjString {
    return if (self.gc.strings.get(string)) |interned|
        self.gc.get(o.ObjString, interned)
...
fn getQualifiedName(self: *Self, node: Ast.Node.Index, raw: bool) ![]const u8 {
    ...
    switch (tag) {
        .Function => {
            ...
            const function_def = self.gc.get(o.ObjTypeDef, type_defs[node].?).resolved_type.?.Function;
            const function_type = function_def.function_type;
            const name = self.gc.get(o.ObjString, function_def.name).string;
...
fn generateString(self: *Self, node: Ast.Node.Index) Error!?m.MIR_op_t {
    ...
    if (elements.len == 0) {
        return m.MIR_new_uint_op(
            self.ctx,
            self.gc.get(o.ObjFunction, self.state.?.closure.function).chunk.constants.items[0].val,
    ...
    for (elements) |element| {
        ...
        const element_type = self.gc.get(o.ObjTypeDef, type_defs[element].?);
...
fn generateNamedVariable(self: *Self, node: Ast.Node.Index) Error!?m.MIR_op_t {
    ...
    const resolved_type_def = self.gc.get(o.ObjTypeDef, type_def.?);
...
fn generateCall(self: *Self, node: Ast.Node.Index) Error!?m.MIR_op_t {
    ...
    if (self.gc.get(o.ObjTypeDef, type_defs[components.callee].?).def_type == .Enum) {
    ...
    const invoked_on = if (dot != null)
        self.gc.get(o.ObjTypeDef, type_defs[node_components[dot.?].Dot.callee].?).def_type
    else
        null;
    ...
    if (invoked_on != null) {
        ...
        switch (invoked_on.?) {
            ...
            .ObjectInstance => instance: {
                ...
                const field = self.gc.get(o.ObjTypeDef, object_type)
                    .resolved_type.?.Object
                    .fields.get(member_lexeme).?;

                if (field.method) {
                    try self.queueObjectMethodCollateral(self.gc.get(o.ObjTypeDef, object_type), field.index);
                }
    ...
    const callee_type_def = switch (node_types[components.callee]) {
        .Dot => node_components[components.callee].Dot.member_type_def,
        else => type_defs[components.callee].?,
    };
    const function_type_def = self.gc.get(o.ObjTypeDef, callee_type_def);
    if (function_type_def.def_type != .Function or function_type_def.optional) {
        return error.CantCompile;
    }
    ...
    if (function_type == .Extern) {
        return try self.generateHandleExternReturn(
            ...
            self.gc.get(o.ObjTypeDef, function_type_def.resolved_type.?.Function.return_type).def_type != .Void,
            m.MIR_new_reg_op(self.ctx, result),
...
fn generateMatchCondition(
    ...
    const type_defs = self.state.?.ast.nodes.items(.type_def);
    const condition_type_def = self.gc.get(o.ObjTypeDef, type_defs[condition].?);
...
fn generateMatch(self: *Self, node: Ast.Node.Index) Error!?m.MIR_op_t {
    ...
    const value_type_def = self.gc.get(o.ObjTypeDef, type_defs[components.value].?);
    const out_label = m.MIR_new_label(self.ctx);
    const match_value = m.MIR_new_reg_op(
        self.ctx,
        try self.REG("match_value", m.MIR_T_I64),
...
fn generateBinary(self: *Self, node: Ast.Node.Index) Error!?m.MIR_op_t {
    ...
    return switch (components.operator) {
        ...
        => try self.generateComparison(components),
            ...
            const type_defs = self.state.?.ast.nodes.items(.type_def);
            const type_def = self.gc.get(o.ObjTypeDef, type_defs[components.left].?).def_type;
...
fn generateComparison(self: *Self, components: Ast.Binary) Error!?m.MIR_op_t {
    ...
    const left_type_def = self.gc.get(o.ObjTypeDef, type_defs[components.left].?).def_type;
    const right_type_def = self.gc.get(o.ObjTypeDef, type_defs[components.right].?).def_type;
...
fn generateDot(self: *Self, node: Ast.Node.Index) Error!?m.MIR_op_t {
    ...
    const callee_type = self.gc.get(o.ObjTypeDef, type_defs[components.callee].?);
        ...
        .Object => {
            switch (components.member_kind) {
                ...
                .Value => {
                    ...
                    try self.buildBinary(
                        assign_token,
                        self.gc.get(o.ObjTypeDef, field.type_def).def_type,
                        res,
                        gen_value,
                        res,
                    );
        ...
        .ObjectInstance, .ProtocolInstance => {
            switch (components.member_kind) {
                ...
                .Value => {
                    ...
                    const field = self.gc.get(o.ObjTypeDef, callee_type.resolved_type.?.ObjectInstance.of)
                    ...
                    const field = if (callee_type.def_type == .ObjectInstance)
                        self.gc.get(o.ObjTypeDef, callee_type.resolved_type.?.ObjectInstance.of)
                            .resolved_type.?.Object.fields
                            .get(member_lexeme)
                    else
                        null;
        ...
        .ForeignContainer => {
            switch (components.member_kind) {
                ...
                .Value => {
                    ...
                    try self.buildBinary(
                        assign_token,
                        self.gc.get(o.ObjTypeDef, field_type_def).def_type,
                        res,
                        gen_value,
                        res,
                    );
...
fn generateAnonymousEnumCase(self: *Self, node: Ast.Node.Index) Error!?m.MIR_op_t {
    ...
    const type_defs = self.state.?.ast.nodes.items(.type_def);
    if (type_defs[node] == null or self.gc.get(o.ObjTypeDef, type_defs[node].?).def_type != .EnumInstance) {
        return error.CantCompile;
    }

    const enum_value = self.gc.get(
        o.ObjTypeDef,
        self.gc.get(o.ObjTypeDef, type_defs[node].?).resolved_type.?.EnumInstance.of,
...
fn generateSubscript(self: *Self, node: Ast.Node.Index) Error!?m.MIR_op_t {
    ...
    switch (self.gc.get(o.ObjTypeDef, type_defs[components.subscripted].?).def_type) {
        .List => {
            ...
            if (value) |val| {
                if (tags[components.assign_token.?] != .Equal) {
                    ...
                    try self.buildBinary(
                        tags[components.assign_token.?],
                        self.gc.get(o.ObjTypeDef, type_defs[components.value.?].?).def_type,
                        res,
                        val,
                        res,
                    );
...
fn objectGlobalSlotForInstanceType(self: *Self, type_def: *o.ObjTypeDef) ?usize {
    ...
    const object_def = self.gc.get(o.ObjTypeDef, type_def.resolved_type.?.ObjectInstance.of)
        .resolved_type.?.Object;
    ...
    for (self.state.?.closure.globals.items, 0..) |global, slot| {
        ...
        const object = o.ObjObject.cast(global.obj(self.gc)) orelse continue;
        const global_object_def = self.gc.get(o.ObjTypeDef, object.type_def).resolved_type.?.Object;
        if (std.mem.eql(
            u8,
            self.gc.get(o.ObjString, object_def.qualified_name).string,
            self.gc.get(o.ObjString, global_object_def.qualified_name).string,
        )) {
            return slot;
        }
    }
...
fn generateObjectInit(self: *Self, node: Ast.Node.Index) Error!?m.MIR_op_t {
    ...
    const type_defs = self.state.?.ast.nodes.items(.type_def);
    const type_def = self.gc.get(o.ObjTypeDef, type_defs[node].?);

    if (type_def.def_type == .ForeignContainer) {
        return self.generateForeignContainerInit(node);
    }

    const object = if (components.object != null and self.gc.get(
        o.ObjTypeDef,
    ...
    for (components.properties) |property| {
        try self.buildExternApiCall(
            ...
            &.{
                ...
                m.MIR_new_uint_op(
                    self.ctx,
                    self.gc.get(o.ObjTypeDef, type_def.resolved_type.?.ObjectInstance.of)
                        .resolved_type.?.Object.fields
    ...
    for (components.properties) |property| {
        try self.buildExternApiCall(
            ...
            &.{
                ...
                m.MIR_new_uint_op(
                    self.ctx,
                    self.gc.get(o.ObjTypeDef, type_def).resolved_type.?.ForeignContainer
                        .fields
...
fn generateUnary(self: *Self, node: Ast.Node.Index) Error!?m.MIR_op_t {
    ...
    switch (components.operator) {
        ...
        .Minus => {
            if (self.gc.get(o.ObjTypeDef, left_type_def.?).def_type == .Integer) {
                try self.unwrap(.Integer, left, result);
...
fn generatePattern(self: *Self, node: Ast.Node.Index) Error!?m.MIR_op_t {
    ...
    for (self.gc.get(o.ObjFunction, self.state.?.closure.function).chunk.constants.items) |constant| {
    ...
    try self.gc.get(o.ObjFunction, self.state.?.closure.function).chunk.constants.append(
        self.gc.allocator,
        value,
    );
...
fn generateForEach(self: *Self, node: Ast.Node.Index) Error!?m.MIR_op_t {
    ...
    const iterable_type_def = self.gc.get(o.ObjTypeDef, self.state.?.ast.nodes.items(.type_def)[components.iterable].?);
...
fn generateFunction(self: *Self, node: Ast.Node.Index) Error!?m.MIR_op_t {
    ...
    if (self.gc.get(
        ...
        self.gc.get(
            o.ObjTypeDef,
            type_defs[self.state.?.ast_node].?,
        ).resolved_type.?.Function.return_type,
    ).def_type == .Void and !self.state.?.return_emitted) {
...
fn generateNativeFn(self: *Self, node: Ast.Node.Index, raw_fn: m.MIR_item_t) !m.MIR_item_t {
    ...
    const should_return = self.gc.get(o.ObjTypeDef, function_def.return_type).def_type != .Void;
...
pub fn compileZdefContainer(self: *Self, ast: Ast.Slice, zdef_element: Ast.Zdef.ZdefElement) Error!void {
    ...
    const foreign_def = self.gc.get(o.ObjTypeDef, zdef_element.zdef.type_def).resolved_type.?.ForeignContainer;
            ...
            for (foreign_def.zig_type.Struct.fields) |field| {
                ...
                try getters.append(
                    ...
                    try self.buildZdefContainerGetter(
                        ...
                        self.gc.get(o.ObjString, foreign_def.name).string,
                        field.name,
                        self.gc.get(o.ObjTypeDef, foreign_def.buzz_type.get(field.name).?),
                        field.type,
                    ),
                );
...
fn buildZdefWrapper(self: *Self, zdef_element: Ast.Zdef.ZdefElement) Error!m.MIR_item_t {
    ...
    const function_def = self.gc.get(o.ObjTypeDef, zdef_element.zdef.type_def).resolved_type.?.Function;
    ...
    while (it.next() != null) : ({
        ...
        try self.buildBuzzValueToZigValue(
            self.gc.get(o.ObjTypeDef, function_def.parameters.get(function_def.parameters.keys()[zidx]).?),
    ...
    if (result) |res| {
        try self.buildZigValueToBuzzValue(
            self.gc.get(o.ObjTypeDef, buzz_return_type),
            zig_return_type,
            res,
            result_value,
        );
    ...
    self.RET(
        m.MIR_new_int_op(
            self.ctx,
            if (self.gc.get(o.ObjTypeDef, function_def.return_type).def_type != .Void)
                1
            else
                0,
        ),
