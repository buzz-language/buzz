...
test "figuring out mir" {
    ...
    // const print_proto = MIR_new_proto_arr(
    //     ctx,
    //     "p_print",
    //     0,
    //     &[_]MIR_type_t{},
    ...
    // const print_import = MIR_new_import(ctx, "print");
    ...
    const ctx_reg = MIR_reg(ctx, "L", func.u.func);
    const stack_top_ptr_base = MIR_new_func_reg(ctx, func.u.func, MIR_T_I64, "stack_top_ptr_base");
    const stack_top_base = MIR_new_func_reg(ctx, func.u.func, MIR_T_I64, "stack_top_base");
    const index = MIR_new_func_reg(ctx, func.u.func, MIR_T_I64, "index");
    ...
    // *[*]Value
    const stack_top_ptr = MIR_new_mem_op(
        ctx,
        MIR_T_P,
        @offsetOf(L, "top"),
        ctx_reg,
    ...
    MOV(
        ctx,
        func,
        MIR_new_reg_op(ctx, stack_top_ptr_base),
        stack_top_ptr,
    );

    // [*]Value
    const stack_top = MIR_new_mem_op(
        ctx,
        MIR_T_P,
        0,
        stack_top_ptr_base,
    ...
    MOV(
        ctx,
        func,
        MIR_new_reg_op(ctx, stack_top_base),
        stack_top,
    );
    ...
    //     \\          export test
    //     \\          import print
    //     \\ p_print: proto u64:addr
    //     \\ test:    func u64, p:L
    //     \\          local i64:idx, i64:base
    //     \\          mov idx, 0
    //     \\          mov base, p:(L)
    //     \\          call p_print, print, base
    //     \\          mov base, p:(base)
    //     \\          call p_print, print, base
    //     \\          call p_print, print, p:(base)
    //     \\          endfunc
    //     \\          endmodule
    //     \\
    //     ,
    ...
    MIR_load_external(ctx, "print", @intToPtr(*anyopaque, @ptrToInt(&print)));
    ...
    var stack = std.heap.c_allocator.alloc(u64, 4) catch unreachable;
    defer std.heap.c_allocator.free(stack);

    var actual_top = @ptrCast([*]u64, stack[0..]);
    var l = L{
        .top = &actual_top,
    };
    l.top.*[0] = 1;
