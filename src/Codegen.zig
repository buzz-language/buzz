...
fn generateAs(self: *Self, node: Ast.Node.Index, breaks: ?*Breaks) Error!GeneratedNode {
    ...
    if (components.force) {
        const type_def_str = try self.gc.get(
            obj.ObjTypeDef,
            type_defs[components.constant].?,
...
fn generateBinary(self: *Self, node: Ast.Node.Index, breaks: ?*Breaks) Error!GeneratedNode {
    ...
    const locations = self.ast.nodes.items(.location);
    const type_defs = self.ast.nodes.items(.type_def);
    const left_type = self.gc.get(obj.ObjTypeDef, type_defs[components.left].?);
...
fn generateCall(self: *Self, node: Ast.Node.Index, breaks: ?*Breaks) Error!GeneratedNode {
    ...
    const callee_type_def = self.gc.get(obj.ObjTypeDef, type_defs[components.callee].?);
    ...
    if (self.ast.nodes.items(.tag)[components.callee] == .Dot) {
        ...
        const field_accessed = self.gc.get(obj.ObjTypeDef, type_defs[dot.callee].?);
    ...
    const callee_type = switch (self.ast.nodes.items(.tag)[components.callee]) {
        .Dot => self.gc.get(obj.ObjTypeDef, node_components[components.callee].Dot.member_type_def),
        else => self.gc.get(obj.ObjTypeDef, type_defs[components.callee].?),
    };
    ...
    const yield_type = self.gc.get(obj.ObjTypeDef, callee_type.resolved_type.?.Function.yield_type);
    ...
    if (!components.is_async) {
        const current_function_typedef = self.gc.get(
            obj.ObjTypeDef,
            type_defs[self.current.?.function_node].?,
        ).resolved_type.?.Function;
        const current_function_type = current_function_typedef.function_type;
        const current_function_yield_type = self.gc.get(obj.ObjTypeDef, current_function_typedef.yield_type);
        switch (current_function_type) {
            ...
            .Function, .Method, .Anonymous => {
                ...
                if (!compatible_void_yield and !current_function_yield_type.strictEql(yield_type)) {
                    self.reporter.reportTypeCheckWithGc(
                        self.gc,
                        .yield_type,
    ...
    for (arg_keys, 0..) |arg_name, pindex| {
        try missing_arguments.put(
            self.gc.allocator,
            self.gc.get(obj.ObjString, arg_name).string,
            pindex,
        );
    }
    ...
    for (components.arguments, 0..) |argument, index| {
        ...
        _ = missing_arguments.orderedRemove(self.gc.get(obj.ObjString, actual_arg_key).string);
        ...
        for (missing_keys) |missing_key| {
            ...
            const default = defaults.get(default_key.toIdx()) orelse default: {
                ...
                _ = try TypeChecker.inferType(
                    ...
                    self.gc,
                    default_node,
                    self.gc.get(obj.ObjTypeDef, parameter_type),
                );
        ...
        while (true) {
            ...
            for (arguments_order_ref.items, 0..) |arg_key, index| {
                const actual_arg_key = if (index == 0 and std.mem.eql(u8, arg_key, "$"))
                    self.gc.get(obj.ObjString, args.keys()[0]).string
                else
                    arg_key;
    ...
    if (components.catch_default) |catch_default| {
        ...
        if (self.current.?.enclosing != null and self.gc.get(
            obj.ObjTypeDef,
            self.current.?.function.?.type_def,
            ...
            for (errors) |error_type| {
                if (self.gc.get(obj.ObjTypeDef, error_type).def_type == .Void) {
                    continue;
                }
                ...
                if (self.gc.get(
                    ...
                    for (handled_types) |handled_type| {
                        if (self.gc.get(obj.ObjTypeDef, handled_type).eql(self.gc.get(obj.ObjTypeDef, error_type))) {
                            handled = true;
                            break;
                        }

                        if (self.gc.get(obj.ObjTypeDef, handled_type).def_type == .Any) {
                            handles_any = true;
                            break;
                        }
                    }
                ...
                if (!handled) {
                    if (self.current.?.try_should_handle != null) {
                        try self.current.?.try_should_handle.?.put(
                            self.gc.allocator,
                            self.gc.get(obj.ObjTypeDef, error_type),
                            locations[components.callee],
                        );
                    } else {
                        try not_handled.append(self.gc.allocator, self.gc.get(obj.ObjTypeDef, error_type));
                    }
                }
    ...
    // Normal call/invoke
    if (invoked) {
        const callee_def_type = self.gc.get(
            obj.ObjTypeDef,
...
fn generateDot(self: *Self, node: Ast.Node.Index, breaks: ?*Breaks) Error!GeneratedNode {
    ...
    const callee_type = self.gc.get(obj.ObjTypeDef, type_defs[components.callee] orelse self.gc.type_registry.any_type);
    ...
    switch (callee_type.def_type) {
        ...
        .ForeignContainer, .ObjectInstance, .Object => {
            ...
            const field = switch (callee_type.def_type) {
                .ObjectInstance => self.gc.get(obj.ObjTypeDef, callee_type.resolved_type.?.ObjectInstance.of)
            ...
            switch (components.member_kind) {
                .Value => {
                    ...
                    const value_type_def = self.gc.get(obj.ObjTypeDef, type_defs[value].?);
                    ...
                    switch (tags[assign_token]) {
                        .PlusEqual => switch (self.gc.get(obj.ObjTypeDef, type_defs[value].?).def_type) {
                        ...
                        .MinusEqual => switch (self.gc.get(obj.ObjTypeDef, type_defs[value].?).def_type) {
                            .Integer => try self.OP_SUBTRACT_I(locations[value]),
                            .Double => try self.OP_SUBTRACT_F(locations[value]),
                            else => {},
                        },
                        .StarEqual => switch (self.gc.get(obj.ObjTypeDef, type_defs[value].?).def_type) {
                            .Integer => try self.OP_MULTIPLY_I(locations[value]),
                            .Double => try self.OP_MULTIPLY_F(locations[value]),
                            else => {},
                        },
                        .SlashEqual => switch (self.gc.get(obj.ObjTypeDef, type_defs[value].?).def_type) {
                        ...
                        .PercentEqual => switch (self.gc.get(obj.ObjTypeDef, type_defs[value].?).def_type) {
...
fn generateAnonymousEnumCase(self: *Self, node: Ast.Node.Index, _: ?*Breaks) Error!GeneratedNode {
    ...
    const enum_type_def = self.gc.get(obj.ObjTypeDef, type_def.?.resolved_type.?.EnumInstance.of).resolved_type.?.Enum;
...
fn generateFor(self: *Self, node: Ast.Node.Index, breaks: ?*Breaks) Error!GeneratedNode {
    ...
    const condition_type_def = self.gc.get(obj.ObjTypeDef, type_defs[components.condition].?);
    ...
    for (components.post_loop) |expr| {
        const expr_type_def = self.gc.get(obj.ObjTypeDef, type_defs[expr].?);
...
fn generateForEach(self: *Self, node: Ast.Node.Index, breaks: ?*Breaks) Error!GeneratedNode {
    ...
    const iterable_type_def = self.gc.get(obj.ObjTypeDef, type_defs[components.iterable].?);
...
fn generateFunction(self: *Self, node: Ast.Node.Index, breaks: ?*Breaks) Error!GeneratedNode {
    ...
    const function_signature = if (components.function_signature) |fs|
        ...
        null;
    const node_type_def = self.gc.get(obj.ObjTypeDef, type_defs[node].?);
        ...
        .Function, .Method, .Anonymous, .Extern => {
            for (self.ast.nodes.items(.components)[components.function_signature.?].FunctionType.arguments) |argument| {
                if (argument.default) |default| {
                    ...
                    const parameter_type = self.gc.get(
                        obj.ObjTypeDef,
                        node_type_def.resolved_type.?.Function.parameters.get(default_name.toIdx()).?,
                    );
                    _ = try TypeChecker.inferType(
                        self.ast,
                        &self.reporter,
    ...
    if (self.gc.get(obj.ObjTypeDef, function.type_def).def_type == .Placeholder) {
        ...
        const function_def = self.gc.get(obj.ObjTypeDef, function.type_def).resolved_type.?.Function;

        if (self.gc.get(obj.ObjTypeDef, function_def.return_type).def_type == .Placeholder) {
            self.reporter.reportPlaceholder(
                self.ast,
                self.gc,
                self.gc.get(
        ...
        if (self.gc.get(obj.ObjTypeDef, function_def.yield_type).def_type == .Placeholder) {
            self.reporter.reportPlaceholder(
                self.ast,
                self.gc,
                self.gc.get(
        ...
        while (it.next()) |kv| {
            if (self.gc.get(obj.ObjTypeDef, kv.value_ptr.*).def_type == .Placeholder) {
                self.reporter.reportPlaceholder(
                    self.ast,
                    self.gc,
                    self.gc.get(
        ...
        if (function_def.error_types) |error_types| {
            for (error_types) |error_type| {
                if (self.gc.get(obj.ObjTypeDef, error_type).def_type == .Placeholder) {
                    self.reporter.reportPlaceholder(
                        self.ast,
                        self.gc,
                        self.gc.get(
    ...
    if (components.slot_type == .Global) {
        try self.OP_DEFINE_GLOBAL(
            ...
            stringValue(
                self.gc.get(
                    obj.ObjTypeDef,
                    self.ast.nodes.items(.type_def)[node].?,
                ).resolved_type.?.Function.name,
            ),
        );
    }
...
fn generateMatch(self: *Self, node: Ast.Node.Index, breaks: ?*Breaks) Error!GeneratedNode {
    ...
    const value_type_def = self.gc.get(obj.ObjTypeDef, type_defs[components.value].?);
    ...
    cases: {
        ...
        for (self.gc.get(
            obj.ObjTypeDef,
            value_type_def.resolved_type.?.EnumInstance.of,
        ).resolved_type.?.Enum.cases, 0..) |case, idx| {
    ...
    for (components.branches, 0..) |branch, bidx| {
        ...
        for (branch.conditions, 0..) |condition, idx| {
            ...
            const condition_type_def = self.gc.get(obj.ObjTypeDef, type_defs[condition].?);
...
fn generateObjectDeclaration(self: *Self, node: Ast.Node.Index, breaks: ?*Breaks) Error!GeneratedNode {
    ...
    const object_type = self.gc.get(obj.ObjTypeDef, type_defs[node].?);
...
fn objectGlobalSlotForInstanceType(self: *Self, type_def: *obj.ObjTypeDef) ?u24 {
    ...
    const object_def = self.gc.get(obj.ObjTypeDef, type_def.resolved_type.?.ObjectInstance.of).resolved_type.?.Object;
    if (object_def.anonymous) {
        return null;
    }

    const current_script = self.gc.get(
        obj.ObjString,
        self.gc.get(obj.ObjTypeDef, self.current.?.function.?.type_def).resolved_type.?.Function.script_name,
    ).string;
    ...
    for (current_globals, 0..) |global, slot| {
        ...
        if (std.mem.eql(
            u8,
            self.gc.get(obj.ObjString, object_def.qualified_name).string,
            self.gc.get(obj.ObjString, global_object_def.qualified_name).string,
        )) {
...
fn generateObjectInit(self: *Self, node: Ast.Node.Index, breaks: ?*Breaks) Error!GeneratedNode {
    ...
    if (components.object != null and self.gc.get(
        obj.ObjTypeDef,
        type_defs[components.object.?].?,
    ).def_type == .Object) {
    ...
    for (components.properties) |property| {
        ...
        const property_idx = if (node_type_def.def_type == .ObjectInstance)
            if (self.gc.get(obj.ObjTypeDef, node_type_def.resolved_type.?.ObjectInstance.of)
                .resolved_type.?.Object
...
fn generateString(self: *Self, node: Ast.Node.Index, breaks: ?*Breaks) Error!GeneratedNode {
    ...
    for (elements, 0..) |element, index| {
        const element_type_def = self.gc.get(obj.ObjTypeDef, type_defs[element].?);
...
fn generateSubscript(self: *Self, node: Ast.Node.Index, breaks: ?*Breaks) Error!GeneratedNode {
    ...
    const subscripted_type_def = self.gc.get(obj.ObjTypeDef, type_defs[components.subscripted].?);
...
fn generateTry(self: *Self, node: Ast.Node.Index, breaks: ?*Breaks) Error!GeneratedNode {
    ...
    for (components.clauses) |clause| {
        const error_type = self.gc.get(obj.ObjTypeDef, type_defs[clause.type_def].?);
    ...
    if (!has_unconditional) {
        ...
        while (it.next()) |kv| {
            ...
            for (components.clauses) |cls| {
                if (self.gc.get(obj.ObjTypeDef, type_defs[cls.type_def].?).eql(kv.key_ptr.*)) {
                    clause = cls;
                    break;
                }
            }
...
fn generateThrow(self: *Self, node: Ast.Node.Index, breaks: ?*Breaks) Error!GeneratedNode {
    ...
    const expression_type_def = self.gc.get(obj.ObjTypeDef, type_defs[components.expression].?);
    const current_error_types = self.gc.get(
        obj.ObjTypeDef,
        self.current.?.function.?.type_def,
    ...
    if (current_error_types != null) {
        for (current_error_types.?) |error_type| {
            if (expression_type_def.eql(self.gc.get(obj.ObjTypeDef, error_type))) {
                found_match = true;
                break;
            }
        }
    ...
    if (!found_match) {
        if (self.current.?.try_should_handle != null) {
            ...
            const error_str = try self.gc.get(
                obj.ObjTypeDef,
                type_defs[components.expression].?,
            ).toStringAlloc(self.gc.allocator, false, self.gc);
            defer self.gc.allocator.free(error_str);
...
fn generateUnary(self: *Self, node: Ast.Node.Index, breaks: ?*Breaks) Error!GeneratedNode {
    ...
    const location = self.ast.nodes.items(.location)[node];
    const expression_type_def = self.gc.get(obj.ObjTypeDef, self.ast.nodes.items(.type_def)[components.expression].?);
...
fn generateZdef(self: *Self, node: Ast.Node.Index, _: ?*Breaks) Error!GeneratedNode {
    ...
    if (self.flavor.resolveDynLib()) {
        for (components.elements) |*element| {
            ...
            switch (self.gc.get(obj.ObjTypeDef, element.zdef.type_def).def_type) {
                .Function => {
