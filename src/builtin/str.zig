...
pub fn trim(ctx: *NativeCtx) c_int {
    ...
    ctx.vm.push((ctx.vm.gc.copyString(trimmed) catch @panic("Out of memory")).toValue());
...
pub fn utf8Codepoints(ctx: *NativeCtx) c_int {
    ...
    if (std.unicode.utf8ValidateSlice(str.string)) {
        ...
        while (it.nextCodepointSlice()) |codepoint| {
            const codepoint_str = ctx.vm.gc.copyString(codepoint) catch @panic("Out of memory");

            list.rawAppend(ctx.vm.gc, codepoint_str.toValue()) catch @panic("Out of memory");
        }
    }
...
pub fn repeat(ctx: *NativeCtx) c_int {
    ...
    var i: usize = 0;
    while (i < n) : (i += 1) {
        new_string.appendSlice(str.string) catch @panic("Out of memory");
    }

    const new_objstring = ctx.vm.gc.copyString(new_string.items) catch @panic("Out of memory");
...
pub fn replace(ctx: *NativeCtx) c_int {
    ...
    ctx.vm.push(
        (ctx.vm.gc.copyString(new_string) catch @panic("Out of memory")).toValue(),
    );
...
pub fn sub(ctx: *NativeCtx) c_int {
    ...
    ctx.vm.push(
        (ctx.vm.gc.copyString(substr) catch @panic("Out of memory")).toValue(),
    );
...
pub fn split(ctx: *NativeCtx) c_int {
    ...
    if (separator.string.len > 0) {
        ...
        while (it.next()) |fragment| {
            const fragment_str = ctx.vm.gc.copyString(fragment) catch @panic("Out of memory");

            list.rawAppend(ctx.vm.gc, fragment_str.toValue()) catch @panic("Out of memory");
        }
    } else {
        for (self.string) |char| {
            const fragment_str = ctx.vm.gc.copyString(&([_]u8{char})) catch @panic("Out of memory");
...
pub fn decodeBase64(ctx: *NativeCtx) c_int {
    ...
    const size = std.base64.standard.Decoder.calcSizeForSlice(str.string) catch {
        ctx.vm.push((ctx.vm.gc.copyString("Could not decode string") catch @panic("Out of memory")).toValue());

        return -1;
    };
    const decoded = ctx.vm.gc.allocator.alloc(u8, size) catch @panic("Out of memory");
    ...
    var new_string = ctx.vm.gc.copyString(decoded) catch @panic("Out of memory");
...
pub fn upper(ctx: *NativeCtx) c_int {
    ...
    var new_str = ctx.vm.gc.allocator.alloc(u8, str.string.len) catch @panic("Out of memory");
    ...
    var obj_string = ctx.vm.gc.copyString(new_str) catch @panic("Out of memory");
...
pub fn hex(ctx: *NativeCtx) c_int {
    ...
    for (str.string) |char| {
        writer.print("{x:0>2}", .{char}) catch @panic("Out of memory");
    }

    var obj_string = ctx.vm.gc.copyString(result.items) catch @panic("Out of memory");
...
pub fn bin(ctx: *NativeCtx) c_int {
    ...
    var result = ctx.vm.gc.allocator.alloc(u8, str.string.len / 2) catch @panic("Out of memory");
    defer ctx.vm.gc.allocator.free(result);

    for (0..result.len) |i| {
        result[i] = std.fmt.parseInt(u8, str.string[(i * 2)..(i * 2 + 2)], 16) catch {
            ctx.vm.push(
                (ctx.vm.gc.copyString("String does not contain valid hex values") catch @panic("Out of memory")).toValue(),
            );
    ...
    var obj_string = ctx.vm.gc.copyString(result) catch @panic("Out of memory");
