...
pub export fn alignOf(ctx: *api.NativeCtx) callconv(.c) c_int {
    var len: usize = 0;
    const zig_type_str = ctx.vm.bz_peek(0).bz_valueToString(&len, ctx.vm).?;
...
pub export fn sizeOfStruct(ctx: *api.NativeCtx) callconv(.c) c_int {
    ...
    ctx.vm.bz_push(
        api.Value.fromInteger(@intCast(type_def.bz_containerTypeSize(ctx.vm))),
    );
...
pub export fn alignOfStruct(ctx: *api.NativeCtx) callconv(.c) c_int {
    ...
    ctx.vm.bz_push(
        api.Value.fromInteger(@intCast(type_def.bz_containerTypeAlign(ctx.vm))),
    );
...
pub export fn rawData(ctx: *api.NativeCtx) callconv(.c) c_int {
    ...
    if (!data.bz_valueIsForeignContainer(ctx.vm)) {
    ...
    var len: usize = 0;
    const data_ptr = data.bz_foreignContainerSlice(&len, ctx.vm);
