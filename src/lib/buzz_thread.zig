...
export fn ThreadSpawn(ctx: *api.NativeCtx) c_int {
    ...
    const thread = api.VM.allocator.create(std.Thread) catch @panic("Out of memory");
...
export fn SemaphoreInit(ctx: *api.NativeCtx) c_int {
    const semaphore = api.VM.allocator.create(std.Thread.Semaphore) catch @panic("Out of memory");
...
export fn initMutex(ctx: *api.NativeCtx) c_int {
    const mutex = api.VM.allocator.create(std.Thread.Mutex) catch @panic("Out of memory");
