...
export fn abs(ctx: *api.NativeCtx) c_int {
    const n_f: f64 = api.Value.bz_valueToFloat(ctx.vm.bz_peek(0));
...
export fn bzceil(ctx: *api.NativeCtx) c_int {
    ...
    ctx.vm.bz_pushInteger(@floatToInt(i32, std.math.ceil(n_f)));
...
export fn bzlog(ctx: *api.NativeCtx) c_int {
    const base_i: f64 = api.Value.bz_valueToFloat(ctx.vm.bz_peek(1));
    const n_f: f64 = api.Value.bz_valueToFloat(ctx.vm.bz_peek(0));
...
export fn max(ctx: *api.NativeCtx) c_int {
    const a_f: f64 = api.Value.bz_valueToFloat(ctx.vm.bz_peek(0));
    const b_f: f64 = api.Value.bz_valueToFloat(ctx.vm.bz_peek(1));
...
export fn bzsin(ctx: *api.NativeCtx) c_int {
    const n: f64 = api.Value.bz_valueToFloat(ctx.vm.bz_peek(0));
...
export fn pow(ctx: *api.NativeCtx) c_int {
    ...
    const n_i: ?i32 = if (n.bz_valueIsInteger()) n.bz_valueToInteger() else null;
    const n_f: ?f64 = if (n.bz_valueIsFloat()) n.bz_valueToFloat() else null;

    const p_i: ?i32 = if (p.bz_valueIsInteger()) p.bz_valueToInteger() else null;
