...
pub const ObjFiber = struct {
    ...
    fn is(self: *Self, type_def: *ObjTypeDef, gc: *GC) bool {
        return type_def.eql(gc.get(ObjTypeDef, self.fiber.type_def).?);
    }
...
pub const ObjList = struct {
    ...
    pub const ListDef = struct {
        ...
        pub fn member(obj_list: Pool(ObjTypeDef).Idx, parser: *Parser, method: []const u8) !?Method {
...
pub const PlaceholderDef = struct {
    ...
    pub fn link(allocator: Allocator, parent: Pool(ObjTypeDef).Idx, child: Pool(ObjTypeDef).Idx, relation: Relation) !void {
