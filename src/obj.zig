...
pub const ObjMap = struct {
    ...
    pub const MapDef = struct {
        ...
        pub fn member(obj_map: Pool(ObjTypeDef).Idx, parser: *Parser, method: []const u8) !?Method {
...
pub const ObjTypeDef = struct {
    ...
    pub fn toStringAlloc(self_idx: Pool(Self).Idx, gc: *GC, qualified: bool) (Allocator.Error || std.fmt.BufPrintError || error{WriteFailed})![]const u8 {
        ...
        try Self.toString(self_idx, &str.writer, gc, qualified);
    ...
    pub fn toString(self_idx: Pool(Self).Idx, gc: *GC, writer: anytype, qualified: bool) (Allocator.Error || std.fmt.BufPrintError || error{WriteFailed})!void {
        try Self.toStringRaw(self_idx, writer, gc, qualified);
    }
    ...
    fn toStringRaw(self_idx: Pool(Self).Idx, gc: *GC, writer: anytype, qualified: bool) (Allocator.Error || std.fmt.BufPrintError || error{WriteFailed})!void {
        ...
        switch (self.def_type) {
            ...
            .Fiber => {
                ...
                try gc.get(ObjTypeDef, self.resolved_type.?.Fiber.return_type).?
                    .toStringRaw(writer, gc, qualified);
                try writer.writeAll(", ");
    ...
    pub fn toParentType(self: Pool(Self).Idx, type_registry: *TypeRegistry) !Pool(Self).Idx {
        return switch (self.def_type) {
    ...
    pub fn eqlTypeUnion(expected: TypeUnion, actual: TypeUnion, gc: *GC) bool {
        ...
        return switch (expected) {
            ...
            .Fiber => gc.get(ObjTypeDef, expected.Fiber.return_type).?
                .eql(actual.Fiber.return_type, gc) and
                gc.get(ObjTypeDef, expected.Fiber.yield_type).?
                    .eql(actual.Fiber.yield_type, gc),

            .ObjectInstance => expected.ObjectInstance.of.eql(actual.ObjectInstance.of, gc) or expected.ObjectInstance.of == actual.ObjectInstance.of,
            .ProtocolInstance => {
                if (actual == .ProtocolInstance) {
                    return expected.ProtocolInstance.of.eql(actual.ProtocolInstance.of, gc) or expected.ProtocolInstance.of == actual.ProtocolInstance.of;
                } else {
            ...
            .EnumInstance => expected.EnumInstance.of.eql(actual.EnumInstance.of, gc),
            ...
            .Object => {
                ...
                while (it.next()) |entry| {
                    if (actual.Object.fields.get(entry.key_ptr.*)) |other_field| {
                        if (!entry.value_ptr.*.eql(other_field, gc)) {
                            return false;
                        }
                    } else {
                        return false;
            ...
            .List => expected.List.item_type.eql(actual.List.item_type, gc),
            .Map => expected.Map.key_type.eql(actual.Map.key_type, gc) and expected.Map.value_type.eql(actual.Map.value_type, gc),
            .Function => {
                // Compare return type
                if (!expected.Function.return_type.eql(actual.Function.return_type, gc)) {
                    return false;
                }

                // Compare yield type
                if (!expected.Function.yield_type.eql(actual.Function.yield_type, gc)) {
                    return false;
                }
                ...
                for (a_keys, 0..) |_, index| {
                    if (!expected.Function.parameters.get(a_keys[index]).?
                        .eql(actual.Function.parameters.get(b_keys[index]).?, gc))
                    {
                        return false;
                    }
                }
