...
pub inline fn ptr(self: *GC, comptime T: type, index: Pool(T).Idx) ?*T {
    return self.pools.get(T, index);
}

pub fn getObj(self: *GC, obj_idx: o.ObjIdx) ?*o.Obj {
    return switch (obj_idx.obj_type) {
        .Range => if (self.ptr(o.ObjRange, .idx(obj_idx.index))) |instance|
            instance.toObj()
        else
            null,
        .String => if (self.ptr(o.ObjString, .idx(obj_idx.index))) |instance|
            instance.toObj()
        else
            null,
        .Pattern => if (self.ptr(o.ObjPattern, .idx(obj_idx.index))) |instance|
            instance.toObj()
        else
            null,
        .Fiber => if (self.ptr(o.ObjFiber, .idx(obj_idx.index))) |instance|
            instance.toObj()
        else
            null,
        .Type => if (self.ptr(o.ObjTypeDef, .idx(obj_idx.index))) |instance|
            instance.toObj()
        else
            null,
        .Object => if (self.ptr(o.ObjObject, .idx(obj_idx.index))) |instance|
            instance.toObj()
        else
            null,
        .Enum => if (self.ptr(o.ObjEnum, .idx(obj_idx.index))) |instance|
            instance.toObj()
        else
            null,
        .ObjectInstance => if (self.ptr(o.ObjObjectInstance, .idx(obj_idx.index))) |instance|
            instance.toObj()
        else
            null,
        .EnumInstance => if (self.ptr(o.ObjEnumInstance, .idx(obj_idx.index))) |instance|
            instance.toObj()
        else
            null,
        .Function => if (self.ptr(o.ObjFunction, .idx(obj_idx.index))) |instance|
            instance.toObj()
        else
            null,
        .UpValue => if (self.ptr(o.ObjUpValue, .idx(obj_idx.index))) |instance|
            instance.toObj()
        else
            null,
        .Closure => if (self.ptr(o.ObjClosure, .idx(obj_idx.index))) |instance|
            instance.toObj()
        else
            null,
        .List => if (self.ptr(o.ObjList, .idx(obj_idx.index))) |instance|
            instance.toObj()
        else
            null,
        .Map => if (self.ptr(o.ObjMap, .idx(obj_idx.index))) |instance|
            instance.toObj()
        else
            null,
        .Bound => if (self.ptr(o.ObjBoundMethod, .idx(obj_idx.index))) |instance|
            instance.toObj()
        else
            null,
        .ForeignContainer => if (self.ptr(o.ObjForeignContainer, .idx(obj_idx.index))) |instance|
            instance.toObj()
        else
            null,
        .UserData => if (self.ptr(o.ObjUserData, .idx(obj_idx.index))) |instance|
            instance.toObj()
        else
            null,
        .Native => if (self.ptr(o.ObjNative, .idx(obj_idx.index))) |instance|
            instance.toObj()
        else
            null,
    };
...
fn freeObj(self: *GC, comptime T: type, index: Pool(T).Idx) (std.mem.Allocator.Error || std.fmt.BufPrintError)!void {
    ...
    self.obj_collected = .{
        .index = index.index,
        .obj_type = index.get(self).obj.obj_type,
    };
    ...
    switch (T) {
        o.ObjString => {
            ...
            _ = self.strings.remove(index.get(self).string);
            freeMany(self, u8, index.get(self).string);
        },
        o.ObjPattern => if (!is_wasm) {
            index.get(self).pattern.free();
        },
        o.ObjTypeDef => {
            const hash = self.type_registry.removeTypeDef(index);
            if (BuildOptions.gc_debug and hash != null) {
                std.log.info(
                    "Removed registered type #{} `{s}`",
                    .{
                        hash.?,
                        index.get(self).toStringAlloc(self.allocator, true) catch unreachable,
                    },
                );
            }

            index.get(self).deinit();
        },
        o.ObjUpValue => if (index.get(self).closed) |value| {
            if (value.isObj()) {
                try self.freeObj(o.ObjUpValue, .{ .index = value.obj().index });
            }
        },
        o.ObjClosure => index.get(self).deinit(self.allocator),
        o.ObjFunction => index.get(self).deinit(),
        o.ObjObjectInstance => {
            // Calling eventual destructor method
            if (index.get(self).object) |object_idx| {
                const object = object_idx.get(self);
                const type_def = object.type_def.get(self);
                if (type_def.resolved_type.?.Object.fields.get("collect")) |field| {
                    if (field.method and !field.static) {
                        ...
                        _ = index.get(self).vm.pop();
                    }
                }
            }
            index.get(self).deinit(self.allocator);
        },
        o.ObjObject => index.get(self).deinit(self.allocator),
        o.ObjList => index.get(self).deinit(self.allocator),
        o.ObjMap => index.get(self).deinit(self.allocator),
        o.ObjEnum => self.allocator.free(index.get(self).cases),
        o.ObjFiber => {
            index.get(self).fiber.deinit();
            self.allocator.destroy(index.get(self).fiber);
        },
        o.ObjForeignContainer => self.freeMany(u8, index.get(self).data),
        o.ObjEnumInstance,
        o.ObjBoundMethod,
        o.ObjNative,
        o.ObjUserData,
...
pub fn markObjDirty(self: *GC, comptime T: type, idx: Pool(T).Idx) !void {
    const instance = idx.get(self);
    if (!instance.obj.dirty and
        (self.obj_collected == null or
            (self.obj_collected.?.obj_type != instance.obj.obj_type or self.obj_collected.?.index != idx.index)))
    {
...
fn blackenObject(self: *GC, obj_idx: o.ObjIdx) !void {
    ...
    try switch (obj.obj_type) {
        .String => self.ptr(o.ObjString, .{ .index = obj_idx.index }).?.mark(self),
        .Pattern => self.ptr(o.ObjPattern, .{ .index = obj_idx.index }).?.mark(self),
        .UserData => self.ptr(o.ObjUserData, .{ .index = obj_idx.index }).?.mark(self),
        .Native => self.ptr(o.ObjNative, .{ .index = obj_idx.index }).?.mark(self),
        .Range => self.ptr(o.ObjRange, .{ .index = obj_idx.index }).?.mark(self),
        .Type => self.ptr(o.ObjTypeDef, .{ .index = obj_idx.index }).?.mark(self),
        .UpValue => self.ptr(o.ObjUpValue, .{ .index = obj_idx.index }).?.mark(self),
        .Closure => self.ptr(o.ObjClosure, .{ .index = obj_idx.index }).?.mark(self),
        .Function => self.ptr(o.ObjFunction, .{ .index = obj_idx.index }).?.mark(self),
        .ObjectInstance => self.ptr(o.ObjObjectInstance, .{ .index = obj_idx.index }).?.mark(self),
        .Object => self.ptr(o.ObjObject, .{ .index = obj_idx.index }).?.mark(self),
        .List => self.ptr(o.ObjList, .{ .index = obj_idx.index }).?.mark(self),
        .Map => self.ptr(o.ObjMap, .{ .index = obj_idx.index }).?.mark(self),
        .Enum => self.ptr(o.ObjEnum, .{ .index = obj_idx.index }).?.mark(self),
        .EnumInstance => self.ptr(o.ObjEnumInstance, .{ .index = obj_idx.index }).?.mark(self),
        .Bound => self.ptr(o.ObjBoundMethod, .{ .index = obj_idx.index }).?.mark(self),
        .Fiber => self.ptr(o.ObjFiber, .{ .index = obj_idx.index }).?.mark(self),
        .ForeignContainer => self.ptr(o.ObjForeignContainer, .{ .index = obj_idx.index }).?.mark(self),
    };
...
pub fn markFiber(self: *GC, fiber: *v.Fiber) !void {
    ...
    while (current_fiber) |ufiber| {
        ...
        if (fiber.open_upvalues) |open_upvalues| {
            ...
            while (upvalue) |unwrapped| : (upvalue = unwrapped.get(self).next) {
