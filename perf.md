...
- [x] `gray_stack` uses `ArrayList` and keeps capacity after high-water marks (`src/GC.zig:44`, `src/GC.zig:344`, `src/GC.zig:701`). That is usually good for throughput, but if a one-time spike causes persistent memory retention, shrink after full collections or cap retained capacity.

#### Threaded GC sweeping
- [ ] Measure whether sweeping is actually dominated by allocator `free`/`destroy` before adding concurrency. `sweep()` currently walks the object list, unlinks dead objects, and calls `freeObj()` (`src/GC.zig:709`, `src/GC.zig:743`, `src/GC.zig:745`). Time those pieces separately: list walk/remove, `freeObj()` dispatch/deinit, string/type-registry removals, object `collect` calls, and the allocator calls inside `free()`/`freeMany()` (`src/GC.zig:258`, `src/GC.zig:278`).
- [ ] If allocator freeing dominates, split sweep into a VM-thread detach phase and a background free phase. The VM thread should finish mark/trace, unlink unreachable objects from `self.objects`, and prevent another GC while the background sweep is active. The sweep thread should only process detached unreachable objects that the VM can no longer access.
- [ ] Keep VM/GC metadata mutation on the VM thread unless explicitly locked. `freeObj()` currently mutates `self.strings` for strings (`src/GC.zig:410`), `self.type_registry.registry` for typedefs (`src/GC.zig:426`, `src/GC.zig:430`), `self.bytes_allocated` through `free()`/`freeMany()`, and `self.obj_collected` (`src/GC.zig:403`). Either perform those detach/accounting steps before handing objects to the sweep thread, or protect them with a dedicated lock/atomic accounting.
- [ ] Do not run Buzz-level collectors on the sweep thread. `ObjectInstance` destruction can invoke a `collect` method through `buzz_api.bz_invoke()` (`src/GC.zig:485`, `src/GC.zig:492`). That path touches VM state and must remain on the VM thread, before the object is handed to a background free worker.
- [ ] Treat allocator thread safety as necessary but not sufficient. Mimalloc can handle concurrent allocation/free, but Buzz-side fields such as `bytes_allocated`, object lists, intern tables, and type registry are not automatically safe. A practical first implementation would have the sweep thread return a `freed_bytes` total and let the VM thread apply GC accounting and next-threshold updates.

### JIT
- [x] `buildExternApiCall()` builds and deinitializes a `full_args` `ArrayList`, but then uses `self.args_buffer` for the actual MIR call (`src/Jit.zig:1711`, `src/Jit.zig:1712`, `src/Jit.zig:1722`, `src/Jit.zig:1734`). The `full_args` list is dead allocation work and should be removed.
